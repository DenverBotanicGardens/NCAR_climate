...
#8/3/2017   
Assume random sampling over modeled area - MaxEnt Merow et al 2013   
```{r}
AlPh384 <- read.csv("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/FieldData_AlPh_over80.csv")

library(ggplot2)
library(ggmap)

colorado <- get_map(location = c(lon = -105.5,lat = 39), zoom = 7)
ggmap(colorado) +
  geom_point(data = AlPh384[AlPh384$decimalLongitude > -110 & 
                              AlPh384$decimalLongitude < -101 &
                              AlPh384$decimalLatitude < 42 &
                              AlPh384$decimalLatitude > 36 &
                              AlPh384$minimumElevationInMeters < 4402 &
                              AlPh384$minimumElevationInMeters > 1010,], 
             aes(x = decimalLongitude, y = decimalLatitude, 
                 colour = minimumElevationInMeters), shape = 21)+
  labs(colour = "Elevation (meters)")
...
ggmap(colorado)+
  geom_point(data = AlPh384[AlPh384$decimalLongitude > -110 & 
                              ...
                              AlPh384$decimalLatitude < 42 &
                              AlPh384$decimalLatitude > 36,], 
             aes(x = decimalLongitude, y = decimalLatitude), alpha=0.5, shape = 21) +
  geom_vline(xintercept=seq(-109.06, -102.04, length.out = 842),size = 0.5, alpha = 0.5)+
  geom_hline(yintercept = seq(36.9951,41.0034,length.out = 481),size = 0.5, alpha = 0.5) # From Template_AlPh.tif properties in QGIS
...
```{r}
r1 <- raster("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/output_test21_30/ApplyGLM2_1/glm_bin_map.tif")

r1
r1@data@max

r2 <- raster("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/output_test21_30/ApplyGLM3_1/glm_bin_map.tif")
r2@data@max


dirs <- list.dirs("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/output_test21_30")

dirs2 <- dirs[dirs %in% grep(paste(c("GLM","MARS","MAXENT"),collapse="|"),
                             dirs, value=TRUE)]

dirs2 <- dirs2[grep("ExpandedOutput", dirs2, invert=TRUE)]
                   


ls1 <- list.files(dirs2[1], pattern="*.tif")
intersect(grep("bin", ls1),grep("aux",ls1, invert=TRUE))
paste(dirs2[1], ls1[intersect(grep("bin", ls1),grep("aux",ls1, invert=TRUE))], sep="/")

r_all <- lapply(dirs2, function(x){
  ls <- list.files(x, pattern="*.tif")
  r <- raster(paste(x,ls[intersect(grep("bin", ls),grep("aux",ls, invert=TRUE))], sep="/"))
  if(r@data@max == 1){
    r
  } else {
    NA
...
rstack <- do.call(stack,r_all[!is.na(r_all)])

r_all[!is.na(r_all)][[1]]@file@name
unlist(strsplit(rstack[[1]]@file@name,"[\\\\]"))[9] # Get the layer from the path file name

class(rstack)
rstack  #Only 8 from three models for 10 different species

layrs <- lapply(r_all[!is.na(r_all)], function(stk){
  unlist(strsplit(stk@file@name,"[\\\\]"))[9]
})

layrs #so only ID 22 with 195 points and ID 24 with 590 points and 27 with 218 points have data
```
...
Need to find where these are saved. set_wd?? 
```{r}
#I'm here! 
here <- "P:/hackathon/Simulations/ClimateLayers/"

precip1920_2005 <- NetCDF2Raster(2,6,1920,2005,paste(here,"b.e11.B20TRC5CNBDRD.f09_g16.",sep=""),
