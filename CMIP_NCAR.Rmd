...
cc_cl_30 <- lapply(1:30, function(modelindex){
    Ycl <- ncvar_get(precl2006_2080[[modelindex]], "PRECL")
    Ycc <- ncvar_get(precc2006_2080[[modelindex]], "PRECC")
...
save(CCCL, file = "CCCL1.RDATA")
save(cc_cl_30, file ="cccl30.RData")

From the rscript
##Need to submit the Rscript as a batch file - only if really big!

#logged in, not in R, type "bsub"

#!/bin/bash
#BSUB -J foreach           # job name to a job
#BSUB -q small             # specified queue must be configured, host, find configured hosts for a que with bqueues -l
#BSUB -n 16
#BSUB -P P86850053
#BSUB -W 00:40              #a runtime limit
#BSUB -o %J.out
#BSUB -e %J.err
# try ctrl+c

# https://www2.cisl.ucar.edu/resources/computational-systems/yellowstone/using-computing-resources/running-jobs#batchscript
#!/bin/tcsh
#
...
```{r}
test.list <- list(array.1, array.2, array.1)
test.list[[1]][,,2]
str(test.list) # as if there were 2 years for three models

#need to bind the models but by year not all years all models
binded <- do.call(abind, test.list) # all years all models
str(binded)
binded[,,3]

# Now, byyear is ordered year 1 for the three models, then year 2 for the three models
cl <- makeCluster(3)
registerDoParallel(cl)
byyear <- foreach(timeindex = 1:2,
                  .combine = "abind3",
                  .packages = "abind") %dopar%
  {
    byyearmodel <- lapply(test.list, function(model1){
    model1[,,timeindex]
    })
    abind(byyearmodel, along=3)
  }
stopCluster(cl)


lapply(1:2, function(year){
  annavg <- apply(binded[,,year], MARGIN = c(1,2), mean)
  annvar <- apply(binded[,,year], MARGIN = c(1,2), var)
  list(annavg, annvar)
})
...
modelavg.raster <- lapply(modelavg_annual, function(model1){
  mean.raster <- raster(as.matrix(model1[,,1]),
                        xmn=0,xmx=358.75,
                        ymn=-90,ymx=90,
                        crs=CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))
  var.raster <- raster(as.matrix(model1[,,2]),
                        xmn=0,xmx=358.75,
                        ymn=-90,ymx=90,
                        crs=CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))
...
image(modelavg_annual[,,3])
plot(modelavg.raster[[5]][[1]])
plot(precc2006_2080.raster[[5]])

plot(raster(modelavg_annual[[1]][,,1], 
            crs=CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0")))

r <- flip(raster(t(as.matrix(modelavg_annual[[1]][,,1], nrow = 192))),direction="y")
e <- extent(c(0,358.75,-90,90))
extent(r) <- e
plot(r)
r #dim are swapped, need 192 rows, 288 col
class(precc2006_2080[[1]])
dim(precc2006_2080.raster[[1]])
```
