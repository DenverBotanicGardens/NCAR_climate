...
yrs <- c("1_74","74_147","148_220","221_293","294_365")  #the ":" made it just give the first number
type <- c("avg","var")

for(k in 1:5){
  for(i in 1:2){
    ncnew <- nc_create("P:/hackathon/Simulations/avgPRISMnc.nc", list(var_prect))
    ncvar_put(ncnew, var_prect, PRISM_season[[k]][,,i], 
              start=c(1,1,1),count=c(nx,ny,1))
    PRISM.raster <- raster("P:/hackathon/Simulations/avgPRISMnc.nc", varname="PRECT")
    writeRaster(PRISM.raster,
                paste("P:/hackathon/Simulations/PRISM",type[i],yrs[k],collapse="",sep=""),
                format = "GTiff")
  }
}
