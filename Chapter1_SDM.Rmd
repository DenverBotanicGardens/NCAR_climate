...
habitatSpecificity <- function(whichones, pathstart, replicates){
  ...
  gridthin <- thin.max(EORpnts, c("x","y"), nrow(habspno))
  convertgridthin <- SpatialPoints(gridthin[,c("x","y")], proj4string = CRS("+proj=utm +zone=13 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0"))
  ...
  habspEOR <- data.frame(SpeciesNum = x, kfold = 1, HerbType = "EOR",gridextract)    
  ...
  # for all species and both error and no and EOR
  habspecificity <- do.call(rbind,lapply(unique(allsphabsp$HerbType), function(HT){
    ellipseoutwhichones <- do.call(rbind, lapply(whichones, function(x){
      ellipseoutfold <- do.call(rbind,lapply(unique(allsphabsp$kfold[allsphabsp$HerbType == HT]), function(k){
   p <- ggplot(allsphabsp[allsphabsp$SpeciesNum==x & allsphabsp$kfold==k & allsphabsp$HerbType==HT,], 
                aes(Comp.1,Comp.2))+
      geom_point()+
      stat_ellipse(segments=201) #default is to draw 51 line segments to make the ellipse
    # get ellipse coordinates
    ...
    # Area of ellipse from semi-major and semi-minor axes which are largest and smallest of dist to center
    habitatspecificity <- pi*min(dist2center)*max(dist2center)
    ellipseoutfold <- data.frame(SpeciesNum = x, kfold = rep, HerbType = HT, habspec = habitatspecificity)
    ellipseoutfold
        }))
      ellipseoutfold
      }))
...
###BACK HERE TO FIX  Get the points for with and  no error
# for all species and EOR
habspecificity <- do.call(rbind,lapply(reallistofTiffs, function(x){
  polys <- l1G1G2and[l1G1G2and$GNAME %in% c(g1g2namesall68$AcceptedName[x],
                                        g1g2namesall68$Taxon[x]),]
  EORpnts <- do.call(rbind, lapply(1:length(polys), function(f){
       outline <- polys@polygons[[f]]@Polygons[[1]]@coords
  ...
  gridthin <- thin.max(EORpnts, c("x","y"), nrow())
