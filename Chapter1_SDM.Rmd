...
pr.bg_with$DataType = "as-is"
pr.bg_asis500000$Background = 5000000
pr.bg_asis500000$DataType = "as-is"

pr.bg_asisall <- rbind(pr.bg_asis, pr.bg_asis500000)

elboth <- ggplot(pr.bg_asisall, aes(y =ElevationResampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
...
asboth <- ggplot(pr.bg_asisall, aes(y =AspectResampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
...
ruboth <- ggplot(pr.bg_asisall, aes(y =RuggedResampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
...
b1both <- ggplot(pr.bg_asisall, aes(y =Bio1Resampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
...
b12both <- ggplot(pr.bg_asisall, aes(y =Bio12Resampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
...
ggsave("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Images/FigureXX_prbgspaceboth.jpg",
       grid.arrange(grobs= list(asboth,ruboth,elboth,b12both,b1both), ncol = 1,
                    left="Species ID"), width = 6, height = 35, units="in", dpi=300)
```


# Test a range of background point differences in varaibility from the presence points. 
    - need to quantify those differences - how??
