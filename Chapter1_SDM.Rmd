...
```{r}
# allsummaries <- lapply(split(nichevaluesbySDM, nichevaluesbySDM$SpNum), function(x){ 
#   quantilesno <- 
#   data.frame(summary(x$no),summary(x$WITH))
#   })
# (summary(nichevaluesbySDM$no[nichevaluesbySDM$SpNum == 1]))
# allsummaries[[1]]

# plot(1:20, ylim = c(min(nichevaluesbySDM$Elevation), (max(nichevaluesbySDM$Elevation)),
#                     pch= "n"))

toplot <- lapply(names(nichevaluesbySDM)[1:5], function(column){
  tenths <- lapply(seq(0.1,1,by = 0.1), function(t){
    nichevaluesbySDM[nichevaluesbySDM$,column]
  })
}))
