...
ggplot(f3, aes(Year, log_odds, colour = Sex)) + 
        geom_line() + 
        geom_smooth(se = TRUE)
```
...
```{r}
# pathstart <- "Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt_herb_herbpluserror/"
precisionplotsno <- lapply(reallistofTiffs, function(i){
                        gc()
                        resultpath <- list.files(path = pathstart, 
...
precisionplotswith <- lapply(reallistofTiffs, function(i){
                        gc()
                        resultpath <- list.files(path = pathstart, 
...
par(mar=c(5.1,4.1,4.1,0.1))
for(i in which(!unlist(lapply(whichsigndiffprec, is.na)))){
  plot(ecdf(sample(precisionplotsno[[i]],10000)),
       main = g1g2namesall68$AcceptedName[reallistofTiffs[i]], xlim = c(0,1), col="red",
       cex.main=2)
...
ggplot(habspe, aes(Area/(1000^2), habspec/1000, shape=HerbType, colour=HerbType))+
  ...
  theme_bw()+
  scale_colour_manual(values = c("black","green","grey70"))+ #  "EOR"       "NoError"   "WithError"
  theme(legend.position = "none")+
  xlab(expression("Area km"^2))+
...
nichevaluesbySDM <- do.call(rbind,lapply(c(reallistofTiffs), function(i){
      ...
      valuesno <- sample(values(rastoutno), 100000)
      quantilesno <- summary(valuesno)
      tenthsno <- (quantilesno[5]-quantilesno[1])/10
      valuesWITH <- sample(values(rastoutWITH), 100000)
      quantilesWITH <- summary(valuesWITH)
      tenthsWITH <- (quantilesWITH[5]-quantilesWITH[1])/10
      
      # Very slow
      notenths <- do.call(rbind,lapply(2:10, function(t){
        D <- overlay(rasterstack, rastoutno, fun=function(x,y){ 
          x[y<((t-1)*tenthsno) | y>(t*tenthsno)] <- NA; x})
        out <- na.omit(values(D))
        if(nrow(data.frame(out))>0){
          out <- data.frame(out,Tenth = t-1, SpNum = i, DataType = "no")
          out
        } else {
          out <- data.frame(layer.1 = NA, layer.2 = NA, layer.3 = NA, layer.4 = NA,
                            layer.5 = NA, Tenth = t-1, SpNum = i, DataType = "no")
          out          
        }
        }))
      
      # notenths <- lapply(2:3, function(t){
      #   tosubset <- rasterstack
      #   tosubset[rastoutno>((t-1)*tenthsno) & rastoutno<=(t*tenthsno)] <- NA
      #   out <- rasterToPoints(tosubset)[,3]
      #   # xy <- rasterToPoints(rastoutno, function(x) x>((t-1)*tenthsno) & x<=(t*tenthsno))
      #   # out <- extract(rasterstack, xy[,1:2])
      #   rasterstack[rastoutno>((t-1)*tenthsno) & rastoutno<=(t*tenthsno)]
      #   
      #   out <- data.frame(out)
      #   out
      # })
      
      withtenths <- do.call(rbind,lapply(2:10, function(t){
