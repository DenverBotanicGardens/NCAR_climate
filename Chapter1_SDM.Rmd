...
maxentrun <- function(whichones, spatialpointsdataframe_herb, numberofReps = 10, 
        ...
        for(x in whichones){
          ...
          if(error == TRUE){
                errorpoints <- pointsspdf
                coordinates(errorpoints) <- 
              ...
              errorpointsout <- do.call(rbind,lapply(1:nrow(errorpoints), function(r){
                ...
                polygns <- spsample(polygns, 300, "stratified")
                proj4string(bgpnts) <- CRS("+proj=utm +zone=13 ellps=NAD83 +ellps=WGS84")
                bgpnts <- spTransform(bgpnts, CRS("+init=epsg:4326") )
...
maxentrun(whichones = atleast12[1], numberofReps = 2, spatialpointsdataframe_herb = spatialpointsdataframe_herb,
          maxentarguments = TRUE, background_points = background_points, filenames = "noFooerror", 
          pathstart = pathstart2)

```

Add the two additional ones, 61 and 66
```{r}
...
forhistofaverage <- function(pathstart, patternmatch){
  stacktoaverage <- lapply(atleast12, function(i){
    rasterstoavg <- list.files(path = pathstart, 
                               pattern = paste("ProbTiffSp",i,patternmatch,sep=""), 
                               full.names=TRUE)
    ...
    writeRaster(ras.mean, paste(pathstart, "AvgTiffSp",i,patternmatch,g1g2names$AcceptedName[i],".tif", sep=""),overwrite=TRUE)
          gc()
    endCluster()
    ras.mean
  })
...
forhistofsd <- function(pathstart, patternmatch){
  stacktoaverage <- lapply(atleast12, function(i){
    rasterstoavg <- list.files(path = pathstart, 
                               pattern = paste("ProbTiffSp",i,patternmatch,sep=""), 
                               full.names=TRUE)
    ...
    writeRaster(ras.mean, paste(pathstart, "SDTiffSp",i,patternmatch,g1g2names$AcceptedName[i],".tif", sep=""),overwrite=TRUE)
          gc()
    endCluster()
    ras.mean
  })
...
noerrorhistsofavg <- accuracyhistsOneperSp(pathstart1, WithorNo = "no")
