...
# Already made
# load("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt_herb_herbpluserror/habspEORandNoandWith.Rda") # need to remake with 5000 and 500000 background areas
...
# Make points (one per grid cell of size rasterstack) that are covered by EOR polygons in equal number of herbarium specimens using thin.max to have them spatial spread 
...
# Check if I need to remake these before just loading - 2019-08-01
...
# Allow for variation in the background points scale after understanding the scale of predictor variables
```{r}
...
# Old one without the EOR points, EOR points need to define which projection - they are different than the herbarium specimens
```{r}
...
```{r}
# atleast12168 <- c()
# for(l in notnull1_68){ 
#   atleast12168[match(l,notnull1_68)] <- 
#     if(nrow(distXsp_noEORgrid1_68[[l]][distXsp_noEORgrid1_68[[l]]$year>1980,])>11){
#       l } else {
#         NA
#         }
# }
...
# pathstart <- ("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt_backgroundanderror/")


maxentrun(whichones = atleast12168, numberofReps = 10, spatialpointsdataframe_herb = distXsp_noEORgrid1_68,
          maxentarguments = FALSE, filenames = "asis_bg500000", pathstart = pathstart, backgroundscale = 500000)


maxentrun(whichones = atleast12168, numberofReps = 10, spatialpointsdataframe_herb = distXsp_noEORgrid1_68,
          error = TRUE, distdistribution = distdistribution,
          maxentarguments = FALSE, filenames = "WITH_bg500000", pathstart = pathstart, backgroundscale = 500000)

patternmatch <- paste("PredictHerb", c("asis_bg500000","WITH_bg500000"), "Sp", sep="")

stitchtogether(atleast12168, pathstart = pathstart, patternmatch = patternmatch[1], rasternames = "Herbasis_bg500000kfold")

stitchtogether(atleast12168, pathstart = pathstart, patternmatch = patternmatch[2], rasternames = "HerbWITH_bg500000kfold")
...
# show the distribution of variability of presence points in the as-is and with error datasets. rug colored by species. 
    - need to calculate the distance from nearest polygon for the added error datasets. 
```{r}
# first attempt
pathstart6266 <- "Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt_herb_herbpluserror/" 

load(paste(pathstart6266,"maxentHerbWITHerror","Sp", 1,"kfold",1, ".Rda",sep=""))
xm@results
xm@presence
xm@absence


load(paste(pathstart6266,"evaluateHerbWITHerror","Sp", 1,"kfold",1, ".Rda",sep=""))
e@presence
e@cor

pr.bg_with <- do.call(rbind,lapply(atleast12168, function(x){
  outno <- do.call(rbind,lapply(1:10, function(rep){
    load(paste(pathstart6266,"maxentHerbWITHerror","Sp", x,"kfold",rep, ".Rda",sep=""))
    pr <- data.frame(PrBg = "pr", ErrorType = "yes", SpeciesNum = x, kfold = rep, xm@presence)
    bg <- data.frame(PrBg = "bg", ErrorType = "yes", SpeciesNum = x, kfold = rep, xm@absence)
    rbind(pr,bg)
    }))
  outno
  }))

pr.bg_asis <- do.call(rbind,lapply(atleast12168, function(x){
  outno <- do.call(rbind,lapply(1:10, function(rep){
    load(paste(pathstart6266,"maxentHerbnoerror","Sp", x,"kfold",rep, ".Rda",sep=""))
    pr <- data.frame(PrBg = "pr", ErrorType = "yes", SpeciesNum = x, kfold = rep, xm@presence)
    bg <- data.frame(PrBg = "bg", ErrorType = "yes", SpeciesNum = x, kfold = rep, xm@absence)
    rbind(pr,bg)
    }))
...
# TO DO go back and use just the smaller scaled DEM layers. 
# Examine the vairaibility of the raster for each covariate   
```{r}
rasterstack

maxValue(rasterstack)
minValue(rasterstack)
# cellStats(rasterstack, mean)

```

    How different are the background points from the presence points
# Examine the amount of varaibility in the presence points.
```{r}

el <- ggplot(pr.bg_asis, aes(y =ElevationResampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
  ylim(minValue(rasterstack)[1], maxValue(rasterstack)[1])+
  # xlab("Species ID")+
  ylab(expression("Elevation"["meters"]))+
  scale_fill_manual(values=c("midnightblue","darkseagreen3"),
                       name="",
                       breaks=c("pr","bg"),
                       labels=c("Presence", "Background"))+
  theme(legend.position="bottom")

as <- ggplot(pr.bg_asis, aes(y =AspectResampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
  ylim(minValue(rasterstack)[2], maxValue(rasterstack)[2])+
  # xlab("Species ID")+
  ylab("Aspect")+
  scale_fill_manual(values=c("midnightblue","darkseagreen3"),
                       name="",
...
ru <- ggplot(pr.bg_asis, aes(y =RuggedResampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
  ylim(minValue(rasterstack)[3], maxValue(rasterstack)[3])+
  # xlab("Species ID")+
  ylab("Ruggedness")+
  scale_fill_manual(values=c("midnightblue","darkseagreen3"),
                       name="",
...
b1 <- ggplot(pr.bg_asis, aes(y =Bio1Resampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
  ylim(minValue(rasterstack)[4], maxValue(rasterstack)[4])+
  # xlab("Species ID")+
  ylab(expression("Bio"["1"]))+
  scale_fill_manual(values=c("midnightblue","darkseagreen3"),
                       name="",
                       breaks=c("pr","bg"),
                       labels=c("Presence", "Background"))+
...
b12 <- ggplot(pr.bg_asis, aes(y =Bio12Resampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
  ylim(minValue(rasterstack)[5], maxValue(rasterstack)[5])+
  ylab(expression("Bio"["12"]))+
  scale_fill_manual(values=c("midnightblue","darkseagreen3"), 
                       name="",
                       breaks=c("pr","bg"),
                       labels=c("Presence", "Background"))+ 
...
ggsave("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Images/FigureXX_prbgspace.jpg",
       grid.arrange(grobs= list(as,ru,el,b12,b1), ncol = 3,
                    left="Species ID"), width = 20, height = 25, units="in", dpi=300)
```




Again but with background points around more area
```{r}
# larger circles for background points
pathstart <- ("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt_backgroundanderror/")

pr.bg_with500000 <- do.call(rbind,lapply(atleast12168, function(x){
  outno <- do.call(rbind,lapply(1:10, function(rep){
    load(paste(pathstart,"maxentHerbWITH_bg500000","Sp", x,"kfold",rep, ".Rda",sep=""))
    pr <- data.frame(PrBg = "pr", ErrorType = "yes", SpeciesNum = x, kfold = rep, xm@presence)
    bg <- data.frame(PrBg = "bg", ErrorType = "yes", SpeciesNum = x, kfold = rep, xm@absence)
    rbind(pr,bg)
    }))
...
pr.bg_asis500000 <- do.call(rbind,lapply(atleast12168, function(x){
  outno <- do.call(rbind,lapply(1:10, function(rep){
    load(paste(pathstart,"maxentHerbasis_bg500000","Sp", x,"kfold",rep, ".Rda",sep=""))
    pr <- data.frame(PrBg = "pr", ErrorType = "yes", SpeciesNum = x, kfold = rep, xm@presence)
    bg <- data.frame(PrBg = "bg", ErrorType = "yes", SpeciesNum = x, kfold = rep, xm@absence)
    rbind(pr,bg)
    }))
...
el5 <- ggplot(pr.bg_asis500000, aes(y =ElevationResampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
...
as5 <- ggplot(pr.bg_asis500000, aes(y =AspectResampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
...
ru5 <- ggplot(pr.bg_asis500000, aes(y =RuggedResampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
...
b15 <- ggplot(pr.bg_asis500000, aes(y =Bio1Resampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
...
b125 <- ggplot(pr.bg_asis500000, aes(y =Bio12Resampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
...
ggsave("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Images/FigureXX_prbgspace5.jpg",
       grid.arrange(grobs= list(as5,ru5,el5,b125,b15), ncol = 1,
                    left="Species ID"), width = 5, height = 25, units="in", dpi=300)
```

```{r}
pr.bg_asis$Background = 5000
pr.bg_asis$DataType = "as-is"
pr.bg_asis500000$Background = 5000000
pr.bg_asis500000$DataType = "as-is"

pr.bg_with$Background = 5000
pr.bg_with$DataType = "with"
pr.bg_with500000$Background = 5000000
pr.bg_with500000$DataType = "with"

pr.bg_all <- rbind(pr.bg_asis, pr.bg_asis500000, pr.bg_with, pr.bg_with500000)

elboth <- ggplot(pr.bg_all, aes(y =ElevationResampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
  ...
  scale_fill_manual(values=c("midnightblue","darkseagreen3"),
                       ...
                       labels=c("Presence", "Background"))+
  theme(legend.position="bottom")+
  facet_wrap(~Background+DataType)

asboth <- ggplot(pr.bg_all, aes(y =AspectResampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
...
ruboth <- ggplot(pr.bg_all, aes(y =RuggedResampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
...
b1both <- ggplot(pr.bg_all, aes(y =Bio1Resampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
...
b12both <- ggplot(pr.bg_all, aes(y =Bio12Resampled_bioclim, x = as.factor(SpeciesNum), fill=PrBg))+
  geom_boxplot()+
  theme_bw()+
  coord_flip()+ 
  xlab("")+
...
ggsave("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Images/FigureXX_prbgspaceboth.jpg",
       grid.arrange(grobs= list(asboth,ruboth,elboth,b12both,b1both), ncol = 5,
                    left="Species ID"), width = 20, height = 10, units="in", dpi=300)
```


# Test a range of background point differences in varaibility from the presence points. 
    - need to quantify those differences - how?? maybe how much space the presence points take up vs. how much space the background ranges take up. 
    - need to project those differences onto geographic space from environmental space     

        1. Set each layer values outside the tolerance range to NA
        2. Find the pixels with no NAs for any layer. 
        Example: ## Construct reproducible data! (Here a raster stack with NA values in each layer) 
...
Ask Andrea about making covariate layers
# test by setting within the center quantile
```{r}

areaandmaps <- do.call(rbind,lapply(atleast12168, function(x){
  asp.r <- rasterstack[[2]]
  ele.r <- rasterstack[[1]]
  rug.r <- rasterstack[[3]]
  bio1.r <- rasterstack[[4]]
  bio12.r <- rasterstack[[5]]

  elev.q <- quantile(pr.bg_all$ElevationResampled_bioclim[pr.bg_all$SpeciesNum==x])
  asp.q <- quantile(pr.bg_all$AspectResampled_bioclim[pr.bg_all$SpeciesNum==x])
  rug.q <- quantile(pr.bg_all$RuggedResampled_bioclim[pr.bg_all$SpeciesNum==x])
  bio1.q <- quantile(pr.bg_all$Bio1Resampled_bioclim[pr.bg_all$SpeciesNum==x])
  bio12.q <- quantile(pr.bg_all$Bio12Resampled_bioclim[pr.bg_all$SpeciesNum==x])
  
  # elev.q <- summary(pr.bg_all$ElevationResampled_bioclim[pr.bg_all$SpeciesNum==x])
  # asp.q <- summary(pr.bg_all$AspectResampled_bioclim[pr.bg_all$SpeciesNum==x])
  # rug.q <- summary(pr.bg_all$RuggedResampled_bioclim[pr.bg_all$SpeciesNum==x])
  # bio1.q <- summary(pr.bg_all$Bio1Resampled_bioclim[pr.bg_all$SpeciesNum==x])
  # bio12.q <- summary(pr.bg_all$Bio12Resampled_bioclim[pr.bg_all$SpeciesNum==x])

  ele.r[ele.r < elev.q[2] | ele.r > elev.q[4]] <- NA
  asp.r[asp.r < asp.q[2] |asp.r > asp.q[4]] <- NA
  rug.r[rug.r < rug.q[2] | rug.r > rug.q[4]] <- NA
  bio1.r[bio1.r < bio1.q[2] |bio1.r > bio1.q[4]] <- NA
  bio12.r[bio12.r < bio12.q[2] |bio12.r > bio12.q[4]] <- NA
  
  # Sums over the group value, different values over the area of the raster; the values are 2023 to 2620
  # So gives the number of cells that have each value - value being the area of each cell
  
  # s.ele <- tapply(area(ele.r), ele.r[], function(x) sum(x, na.rm = TRUE))
  # sum(s.ele)

  s <- stack(ele.r, asp.r, rug.r, bio1.r, bio12.r)
  s2 <- mask(s, calc(s, fun=sum))
...
# plot(s2)  
  ...
  herbpoints <- SpatialPointsDataFrame(coords = distXsp_noEORgrid1_68[[x]][,c("decimalLongitude","decimalLatitude")],
                                       data = distXsp_noEORgrid1_68[[x]],
                                       proj4string = CRS("+proj=utm +zone=13 ellps=NAD83 +ellps=WGS84"))    
  herbpoints <- spTransform(herbpoints, CRS("+proj=longlat +datum=WGS84"))
  
  # plot(s2[[1]]); plot(polys, add=TRUE); points(herbpoints,pch=21, col="red", cex=2)
  # 
  # Sizes of all cells
  suitablearea <- area(s2, na.rm=TRUE, weights = FALSE)
  areasout <- sum(values(suitablearea), na.rm = TRUE)
  gc()
  out <- data.frame(SuitableArea = areasout, SpNum = x, ElevationAvail = sum(values(area(ele.r))),
                    AspectAvail = sum(values(area(asp.r))),
                    RuggedAvail = sum(values(area(rug.r))),
                    Bio1Avail = sum(values(area(bio1.r))),
                    Bio12Avail = sum(values(area(bio12.r))))
                    
  out
}))

save(areaandmaps, file= "Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Data/areaandmaps.Rda")
...
hist(distdistribution)

pointsspdf <- SpatialPointsDataFrame(coords = distXsp_noEORgrid68[[x]][,c("decimalLongitude","decimalLatitude")],
                                                 data = distXsp_noEORgrid68[[x]],
                                                 proj4string = CRS("+proj=utm +zone=13 ellps=NAD83 +ellps=WGS84"))
...
errorpointsout <- do.call(rbind,lapply(1:nrow(pointsspdf), function(r){
  ...
  out
      ...
      df <- SpatialPointsDataFrame(coords = errorpointsout,
                                   data = pointsspdf@data,
                                   proj4string = CRS("+proj=utm +zone=13 ellps=NAD83 +ellps=WGS84"))
      circlesout <- circles(df, d = 50000) #Should be 5km around
      polygns <- polygons(circlesout)
      bgpnts <- spsample(polygns, 300, "stratified") # one single random location in each 'cell' 
...
notnull1_68 <- which(!unlist(lapply(distXsp_EOR, is.null)))
length(notnull1_68) # 52 have data
...
# Differences in creating distXsp_EOR in Chapter1_EOR_AsIs_Error.Rmd compared to making other distXsp in Chapter1_SDM resulted in some errors in capturing EOR polygons, likely due to unknown mapped years, those should be thrown out. Got rid of 9999 in the  Herb to EORs_v4.Rmd. Need to fix that in distXsp_EOR

# 2019-08-01 after running Chapter1_SDM1_EOR_AsIs_Error.RMD
df_dist <- do.call(rbind,Map(cbind, mapply('[[', distXsp_EOR, 1)[notnull], SpNum = notnull))
head(df_dist)

# Thelypodiopsis juniperorum has no years for the EORs
l1G1G2and[l1G1G2and$GNAME == "Thelypodiopsis juniperorum",]@data$LASTOBS # many after 1980 but one 9999
l1G1G2and[l1G1G2and$GNAME == "Thelypodiopsis juniperorum",]@data$FIRSTOBS # some after 1980 and one 9999

plot(colocounties.UTM)
plot(l1G1G2and[l1G1G2and$GNAME == "Thelypodiopsis juniperorum",], add=TRUE, border="blue") 

# smallest range by polygons from CNHP
plot(colocounties.UTM)
plot(l1G1G2and[l1G1G2and$GNAME == "Packera mancosana",], add=TRUE, border="blue", lwd=10) 

# largest range by polygons from CNHP
plot(colocounties.UTM)
plot(l1G1G2and[l1G1G2and$GNAME == "Eriogonum coloradense",], add=TRUE, border="blue", lwd=10) 


# plot the scale of raster layers and plants and colorado
plot(rasterstack[[1]]) #Elevation
plot(colocounties, add=TRUE)

ErCoUTM <- l1G1G2and[l1G1G2and$GNAME == "Eriogonum coloradense",]
proj4string(ErCoUTM) <- CRS("+proj=utm +zone=13 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0")
ErCoLatLon <- spTransform(ErCoUTM,CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))


# plot the scale of raster layers and plants and colorado
plot(rasterstack[[4]], xlim = c(ErCoLatLon@bbox[1,1]-.5,ErCoLatLon@bbox[1,2]+.5),
     ylim = c(ErCoLatLon@bbox[2,1]-.5,ErCoLatLon@bbox[2,2]+.5)) #Bio1
plot(colocounties, add=TRUE)
plot(ErCoLatLon, add=TRUE, border="blue")
...
```{r}
table(as.numeric(substring(l1G1G2and$LASTOBS, 1,4))) # 4 are 9999

df_dist$EORYear <- as.numeric(substring(df_dist$EORdate,1,4))
table(df_dist$EORYear) 
df_dist$EORYear[df_dist$EORYear == 9999] <- NA

# SpNum added when distXspall was created
...
# write.csv(tab1, "Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Images/table1_20190801.csv")
...
How does the year span (MinEORyear and MaxEORyear) compare to MinYear and MaxYear of the herbarium specimens?   
If I use tidyr then need to call raster::extract and RCurl::complete. Don't know the RCurl one but for sure use the raster::extract    
```{r}
library(tidyr)
yearrangediff <- gather(tab1, YearMeasurment, Year, MinEORyear:MaxYear, factor_key = TRUE)
 # to treat new key column as a factor, not character

ggplot(yearrangediff, aes(Year, Species, colour = as.factor(SpNum1)))+
  geom_point()+
  theme_bw()+
  facet_wrap(~YearMeasurment, nrow=2)+ 
  theme(legend.position="none")

ggplot(tab1, aes(YearDiffAvg.Yeardiff, Area, colour = as.factor(SpNum1)))+
  geom_point()+
  theme_bw()+
  theme(legend.position="none")
...
# Not each of the replicates but the average or the SD of the replicates, one per species
# Add in the overlap of the rasterstack 
accuracyhistsOneperSp_climate <- function(pathstart, whichones = reallistofTiffs, SDorAVG = "Avg", WithorNo = "WITH"){
  lapply(whichones, function(i){
    resultpath <- list.files(path = pathstart, 
    ...
    # environr <- calc(rasterstack, function(x) extract(x, f, small= TRUE, weights = TRUE))
    environr <- lapply(1:5, function(x){
      out <- extract(rasterstack[[x]], f, small=TRUE, weights = TRUE)
      do.call(rbind, out)
    })
    outenviron <- do.call(cbind, environr)
    newnames <- mapply(function(x,y) paste(x,y,sep="_"),x = rep(c("value","weight"),5),y = rep(names(rasterstack),each=2))
    rasterstackpoly <- data.frame(outenviron)
    names(rasterstackpoly) <- newnames
    # Want the value [,1] for each polygon but use the weight [,2] to get the pro-rated cells 
    out <- do.call(rbind,fr)
    out <- data.frame(out)
    data.frame(out,rasterstackpoly)
    })
}
```
...
#Averaged and SD of probability maps second function
# noerrorhistsofavg_withclimate <- accuracyhistsOneperSp_climate(whichones = reallistofTiffs, pathstart1, WithorNo = "no")
# witherrorhistsofavg_withclimate <- accuracyhistsOneperSp_climate(whichones = reallistofTiffs,pathstart1)
# 
# save(noerrorhistsofavg_withclimate, file = "Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt_herb_herbpluserror/noerrorhistsofavg_withclimate.Rdata")
# 
# save(witherrorhistsofavg_withclimate, file = "Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt_herb_herbpluserror/witherrorhistsofavg_withclimate.Rdata")


load("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt_herb_herbpluserror/noerrorhistsofavg_withclimate.Rdata")
load("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt_herb_herbpluserror/witherrorhistsofavg_withclimate.Rdata")
...
#Which are different for climate and slope things?
ks.elev <- lapply(1:length(witherrorhistsofavg_withclimate), function(i){
  ks.test(log(data.frame(noerrorhistsofavg_withclimate[[i]])$value_ElevationResampled_bioclim)+
             log(data.frame(noerrorhistsofavg_withclimate[[i]])$weight_ElevationResampled_bioclim),
          log(data.frame(witherrorhistsofavg_withclimate[[i]])$value_ElevationResampled_bioclim)+
             log(data.frame(witherrorhistsofavg_withclimate[[i]])$weight_ElevationResampled_bioclim))
})

ks.asp <- lapply(1:length(witherrorhistsofavg_withclimate), function(i){
  ks.test(log(data.frame(noerrorhistsofavg_withclimate[[i]])$value_AspectResampled_bioclim)+
             log(data.frame(noerrorhistsofavg_withclimate[[i]])$weight_AspectResampled_bioclim),
          log(data.frame(witherrorhistsofavg_withclimate[[i]])$value_AspectResampled_bioclim)+
             log(data.frame(witherrorhistsofavg_withclimate[[i]])$weight_AspectResampled_bioclim))
})

ks.rugged <- lapply(1:length(witherrorhistsofavg_withclimate), function(i){
  ks.test(log(data.frame(noerrorhistsofavg_withclimate[[i]])$value_RuggedResampled_bioclim)+
             log(data.frame(noerrorhistsofavg_withclimate[[i]])$weight_RuggedResampled_bioclim),
          log(data.frame(witherrorhistsofavg_withclimate[[i]])$value_RuggedResampled_bioclim)+
             log(data.frame(witherrorhistsofavg_withclimate[[i]])$weight_RuggedResampled_bioclim))
})

ks.bio1 <- lapply(1:length(witherrorhistsofavg_withclimate), function(i){
  ks.test(log(data.frame(noerrorhistsofavg_withclimate[[i]])$value_Bio1Resampled_bioclim)+
             log(data.frame(noerrorhistsofavg_withclimate[[i]])$weight_Bio1Resampled_bioclim),
          log(data.frame(witherrorhistsofavg_withclimate[[i]])$value_Bio1Resampled_bioclim)+
             log(data.frame(witherrorhistsofavg_withclimate[[i]])$weight_Bio1Resampled_bioclim))
})

ks.bio12 <- lapply(1:length(witherrorhistsofavg_withclimate), function(i){
  ks.test(log(data.frame(noerrorhistsofavg_withclimate[[i]])$value_Bio12Resampled_bioclim)+
             log(data.frame(noerrorhistsofavg_withclimate[[i]])$weight_Bio12Resampled_bioclim),
          log(data.frame(witherrorhistsofavg_withclimate[[i]])$value_Bio12Resampled_bioclim)+
             log(data.frame(witherrorhistsofavg_withclimate[[i]])$weight_Bio12Resampled_bioclim))
})

# *noerrorhistsofavg_withclimate[[i]]$weight_AspectResampled_bioclim
# *noerrorhistsofavg_withclimate[[i]]$weight
# *witherrorhistsofavg_withclimate[[i]]$weight_AspectResampled_bioclim
# *witherrorhistsofavg_withclimate[[i]]$weight

for(i in 1:length(witherrorhistsofavg_withclimate)){
  plot(noerrorhistsofavg_withclimate[[i]]$value_AspectResampled_bioclim,
       noerrorhistsofavg_withclimate[[i]]$value,
       xlab = "Aspect",
       ylab= "Suitability", pch=16,
       main = g1g2namesall68$AcceptedName[reallistofTiffs[i]])
  points(witherrorhistsofavg_withclimate[[i]]$value_AspectResampled_bioclim,
       witherrorhistsofavg_withclimate[[i]]$value, col="grey50", pch=24)
}
```
...
for(i in 1:length(witherrorhistsofavg_withclimate)){
  plot <- ggplot()+
    geom_point(data=noerrorhistsofavg_withclimate[[i]], aes(value_RuggedResampled_bioclim,value))+
    stat_smooth(data=noerrorhistsofavg_withclimate[[i]], aes(value_RuggedResampled_bioclim,value),
                colour = "black")+
    
    geom_point(data=witherrorhistsofavg_withclimate[[i]], aes(value_RuggedResampled_bioclim,value), 
               colour= "grey50", pch=24)+
    stat_smooth(data=witherrorhistsofavg_withclimate[[i]], aes(value_RuggedResampled_bioclim,value),
                colour = "grey30")+
    xlab("Ruggedness")+
    ylab("Suitability")+
    labs(title = paste(g1g2namesall68$AcceptedName[reallistofTiffs[i]]))+
    theme_bw()
  # scatter.smooth(witherrorhistsofavg_withclimate[[i]]$value_ElevationResampled_bioclim,
  #      witherrorhistsofavg_withclimate[[i]]$value, col="grey50", pch=24, new = FALSE)
  print(plot)
}
...
for(i in 1:length(witherrorhistsofavg_withclimate)){
  plot <- ggplot()+
    geom_point(data=noerrorhistsofavg_withclimate[[i]], aes(value_ElevationResampled_bioclim,value))+
    stat_smooth(data=noerrorhistsofavg_withclimate[[i]], aes(value_ElevationResampled_bioclim,value),
                colour = "black")+
    
    geom_point(data=witherrorhistsofavg_withclimate[[i]], aes(value_ElevationResampled_bioclim,value), 
               colour= "grey50", pch=24)+
    stat_smooth(data=witherrorhistsofavg_withclimate[[i]], aes(value_ElevationResampled_bioclim,value),
                colour = "grey30")+
    xlab("Elevation")+
    ylab("Suitability")+
    labs(title = paste(g1g2namesall68$AcceptedName[reallistofTiffs[i]]))+
...
```{r}
noerrorhistsofavg_withclimate[[4]]
witherrorhistsofavg_withclimate[[4]]
```


```{r}
lapply(grep("value",names(noerrorhistsofavg_withclimate[[i]]))[-1], function(col){
  
  lapply(1:length(witherrorhistsofavg_withclimate), function(i){
    noerrnow <- noerrorhistsofavg_withclimate[[i]][,c(1,col)]
    names(noerrnow) <- c("value","now")
    witherrnow <- witherrorhistsofavg_withclimate[[i]][,c(1,col)]
    names(witherrnow) <- c("value","now")
    plot <- ggplot()+
      geom_point(data=noerrnow, aes(now,value), colour = "blue", pch=21)+
      # geom_boxplot(data=noerrnow, aes(now,value))+
      stat_smooth(data=noerrnow, method = lm, formula = y ~ splines::bs(x, 3), se = FALSE,
                  aes(now,value),colour = "black")+
      
      geom_point(data=witherrnow, aes(now,value), 
                 colour= "grey50", pch=24)+
      geom_smooth(data=witherrnow, method = lm, formula = y ~ splines::bs(x, 3), se = FALSE, aes(now,value),
                  colour = "grey30")+
      xlab(paste(names(noerrorhistsofavg_withclimate[[i]][col])))+
      ylab("Suitability")+
      labs(title = paste(g1g2namesall68$AcceptedName[reallistofTiffs[i]]))+
      theme_bw()
  print(plot)
...
# Regardless of how the suitability changes with the climatic variables, how much does the suitability vary in the two surfaces where the species is known to be found.   
Look for linear relationship and the slope, is there a difference in slope of two lines or are lines flat so there's no discerning if there are differences across an environmental variable? 
```{r}
...
head(distXspall52)
(habspEORandNoandWith)


suittoplot <- do.call(rbind,lapply(1:length(witherrorhistsofavg_withclimate), function(i){
  noout <- data.frame(Species = g1g2namesall68$AcceptedName[reallistofTiffs[i]],
                    Error = "as-is", noerrorhistsofavg_withclimate[[i]])
  # bind the species specificity and area to this table.
  noout <- cbind(noout, 
                 unique(distXspall52[distXspall52$Species == g1g2namesall68$AcceptedName[reallistofTiffs[i]],c("SpNum","Species","Area")]))
  noout <- merge(noout, habspEORandNoandWith[habspEORandNoandWith$HerbType == "NoError",],
                 by.x = "SpNum", by.y = "SpeciesNum")
                 
  
  addedout <- data.frame(Species = g1g2namesall68$AcceptedName[reallistofTiffs[i]],
                    Error = "additional", witherrorhistsofavg_withclimate[[i]])
  addedout <- cbind(addedout, 
                 unique(distXspall52[distXspall52$Species == g1g2namesall68$AcceptedName[reallistofTiffs[i]],c("SpNum","Species","Area")]))
  addedout <- merge(addedout,habspEORandNoandWith[habspEORandNoandWith$HerbType == "WithError",],
                    by.x = "SpNum",by.y = "SpeciesNum")
  
  out <- rbind(noout, addedout)
  out
}))
...
ggplot(suittoplot, aes(x = Error, y=value, colour=Area))+
  geom_boxplot()+
  # geom_jitter(position=position_jitter(0.2))+
  ylab("Suitability")+
  theme_bw()

ggplot(suittoplot, aes(x =Species , y=value, colour=Error))+
  geom_boxplot()+
  # geom_jitter(position=position_jitter(0.2))+
  ylab("Suitability")+
  theme_bw()+ 
...
lapply(1:length(witherrorhistsofavg_withclimate), function(i){
  lapply(grep("value",names(noerrorhistsofavg_withclimate[[i]]))[-1], function(col){
    print(col)
    difference.df <- data.frame(noerrorhistsofavg_withclimate[[i]]$value-
                                  witherrorhistsofavg_withclimate[[i]]$value,
                 noerrorhistsofavg_withclimate[[i]][,col])
    names(difference.df) <- c("Diff","climate")
  
  plot <- ggplot(difference.df, aes(climate, Diff))+
    geom_point()+
    stat_smooth(colour = "black")+
    xlab(paste(names(noerrorhistsofavg_withclimate[[i]][col])))+
    ylab("Difference in Suitability (as-is - additional)")+
    labs(title = paste(g1g2namesall68$AcceptedName[reallistofTiffs[i]]))+
    theme_bw()
  # scatter.smooth(witherrorhistsofavg_withclimate[[i]]$value_ElevationResampled_bioclim,
  #      witherrorhistsofavg_withclimate[[i]]$value, col="grey50", pch=24, new = FALSE)
...
```{r}
for(i in 1:length(witherrorhistsofavg_withclimate)){
  plot(noerrorhistsofavg_withclimate[[i]]$value_RuggedResampled_bioclim,
       noerrorhistsofavg_withclimate[[i]]$value,
       xlab = "Ruggedness",
       ylab= "Suitability", pch=16,
       main = g1g2namesall68$AcceptedName[reallistofTiffs[i]])
  points(witherrorhistsofavg_withclimate[[i]]$value_RuggedResampled_bioclim,
       witherrorhistsofavg_withclimate[[i]]$value, col="grey50", pch=24)
}
...
```{r}
for(i in 1:length(witherrorhistsofavg_withclimate)){
  plot(noerrorhistsofavg_withclimate[[i]]$value_Bio1Resampled_bioclim,
       noerrorhistsofavg_withclimate[[i]]$value,
       xlab = "Bio1",
       ylab= "Suitability", pch=16,
       main = g1g2namesall68$AcceptedName[reallistofTiffs[i]])
  points(witherrorhistsofavg_withclimate[[i]]$value_Bio1Resampled_bioclim,
       witherrorhistsofavg_withclimate[[i]]$value, col="grey50", pch=24)
}
...
sort(unique(df_dist$Area[df_dist$SpNum %in% reallistofTiffs[which(!is.na(whichsigndiff))]]))/(1000^2)
areasortaccuracy <- unique(df_dist[df_dist$SpNum %in% reallistofTiffs[which(!is.na(whichsigndiff))], 
                                                              c("SpNum","Area")])

areasortaccuracy <- areasortaccuracy[order(areasortaccuracy$Area),"SpNum"]
# This worked for areasort with precision because all were signficantly different so no NAs
accuracyspeciesordered <- reallistofTiffs[which(!is.na(whichsigndiff))][match(reallistofTiffs ,areasortaccuracy)[!is.na(match(reallistofTiffs ,areasortaccuracy))]]

match(reallistofTiffs[which(!is.na(whichsigndiff))],accuracyspeciesordered)
length(reallistofTiffs[which(!is.na(whichsigndiff))])
length(accuracyspeciesordered)
areasortaccuracy <- c(1:33)[which(!is.na(whichsigndiff))][match(reallistofTiffs ,areasortaccuracy)[!is.na(match(reallistofTiffs ,areasortaccuracy))]]


# What if I sorted by sample size?
tab1$DistAvg.N  # This is with all, not just after 1980!

SDM.N <- do.call(rbind, lapply(split(df_dist, df_dist$SpNum), function(y){
  x <- y[y$year >1980,]
  data.frame(SpNum1 = unique(x$SpNum), Species = x$Species[1], DistAvg = summarySE(x, "Dist"), 
             Area = unique(x$Area), minYearDiff = min(x$Yeardiff), maxYearDiff = max(x$Yeardiff),
             SpNum= unique(x$SpNum), YearDiffAvg = summarySE(x, "Yeardiff"),
...
write.csv(SDM.N, "Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Images/SDMSamplesize.csv")

sort(unique(SDM.N$DistAvg.N[SDM.N$SpNum1 %in% reallistofTiffs[which(!is.na(whichsigndiff))]]))
areasortaccuracy <- unique(df_dist[df_dist$SpNum %in% reallistofTiffs[which(!is.na(whichsigndiff))], 
                                                              c("SpNum","Area")])
...
```
 ...
 sorting is not working. But want to look at the distribution of suitability according to climate variables. 
```{r}
...
for(i in 1:length(areasortaccuracy)){
  noerr <- data.frame(noerrorhistsofavg[[areasortaccuracy[i]]])
  # noerr <- noerr[sample(1:nrow(noerr),1000),]
  witherr <- data.frame(witherrorhistsofavg[[areasortaccuracy[i]]])
  ...
  #            log(data.frame(witherrorhistsofavg[[i]])$weight))
  plot(as_is, col = "blue", 
       main=bquote(.(g1g2namesall68$AcceptedName[accuracyspeciesordered[i]]) ~
                     .(round(unique(df_dist$Area[df_dist$SpNum==accuracyspeciesordered[i]])/(1000^2),0)) ~
                     km^2),
       cex.main=2)  
...
#Cummulative Distribution curve of the raster cells against the climate variables to compare as-is to with error. 
```{r}
