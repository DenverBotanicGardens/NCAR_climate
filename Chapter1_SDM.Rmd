...
nichevaluesbySDM <- do.call(rbind,lapply(c(reallistofTiffs), function(i){
      ...
      # valuesno <- values(rastoutno)
      # quantilesno <- summary(valuesno)
      # tenthsno <- (max(valuesno)-min(valuesno))/4
      # valuesWITH <- values(rastoutWITH)
      # quantilesWITH <- summary(valuesWITH)
      # tenthsWITH <- (max(valuesWITH)-min(valuesWITH))/4

      # Very slow
      # notenths <- do.call(rbind,lapply(2:4, function(t){
      #   gc()
        D <- overlay(stack(rasterstack,rastoutno), rastoutno, fun=function(x,y){
          x[sample(y, 10000)]
          # x[y>(t*tenthsno)] <- NA
          # x
          })
      ...
      data.frame(no = valuesno, WITH = valuesWITH, SpNum = i)
}))
```
