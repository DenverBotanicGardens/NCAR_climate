...
suittoplot <- do.call(rbind,lapply(1:length(witherrorhistsofavg_withclimate), function(i){
  ...
  noout <- cbind(noout, 
                 distXspall52[distXspall52$Species == g1g2namesall68$AcceptedName[reallistofTiffs[i]],])
                              # &distXspall52$year > 1980,])
...
# unlist(lapply(distXsp_noEORgrid68, function(x){
#   print(nrow(x))
# }))

distXsp_noEORgrid68[[1]] # There are way fewer... 
distXsp_noEORgrid1_68[[1]] 

unlist(lapply(distXsp_noEORgrid1_68, function(x){
  print(nrow(x))
}))
```

```{r}
lapply(noerrorhistsofavg_withclimate, function(x){
  print(nrow(x))
})
...
```{r}
ggplot(suittoplot, aes(x = Error, y=value, colour=Species))+
  geom_boxplot()+
