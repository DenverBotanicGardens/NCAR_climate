...
Make EOR with as many datapoints as you can, all that fit in a separate cell? Or make the combination of all the Herbarium and all the EORs? That first, lots of data points later, compare same number of datapoints/sample size, then compare more. Combine all points and then spatially thin to make it even? Or compare keeping all to keeping only the limiting number? 
```{r}
distXsp_EORmaxpoints <- lapply(1:nrow(g1g2namesall68), function(i){
  gc()
  g1g2now <- coloradosps.g1g2_168[coloradosps.g1g2_168$scientificName %in%
                                c(g1g2namesall68$Taxon[i],g1g2namesall68$AcceptedName[i])&
                                !is.na(coloradosps.g1g2_168$decimalLatitude),]
...
save(distXsp_EORmaxpoints, file= "Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Data/distXsp_EOR.Rda") 
...
g1g2now <- g1g2now[,c("scientificName","scientificNameAuthorship","institutionCode", 
                        "recordedBy","year","decimalLatitude","decimalLongitude")] # ,"lat2","lon2"
```
5, 7,8, 11 8 is good but none outside
```{r}
plot(toplot[c(7),], border="red", lwd=2)
points(g1g2now$decimalLongitude, g1g2now$decimalLatitude, pch=16, cex = 0.5)
...
load habitat specificiy from file 
```{r}
load("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt_herb_herbpluserror/habSp_EOR_uncleaned_error.Rda") 

# to merge with table 1
summaryHabSp <- summarySE(habSp_EOR_uncleaned_error[habSp_EOR_uncleaned_error$HerbType == "As-Is_bg5000",],
                          "habspec", "SpeciesNum")
...
habSp_EOR_uncleaned_error <- habitatSpecificity(whichones = atleast12[atleast12 != 8], 
                                                pathstart = pathstart,
                                                filenames = filenames, rasterstack = rasterstack)
save(habSp_EOR_uncleaned_error, file= "Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt_herb_herbpluserror/habSp_EOR_uncleaned_error.Rda")
...
maxevaluates <- do.call(rbind,lapply(atleast12, function(x){
  outnow <- do.call(rbind,lapply(c(filenames,filenamesbig), function(filenames){
...
ggplot(maxevaluates, aes(y = AUC, fill = filename))+
  geom_boxplot()+
  theme_bw()+
  scale_fill_manual(values = c("grey","white","black","grey","white","black"),
                    name = "Dataset / background",
                    breaks = c(paste(filenames), paste(filenamesbig)),
                    labels = c("EOR / small",
                               "uncleaned / small",
                               "error / small","EOR / big","uncleaned / big","error / big"))
...
jpeg("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Images/Figure5_nicheOverlap.jpg",
...
ggplot(overlap_values, aes(variable, value, fill = Background))+
  ...
  scale_fill_manual(values = c("white","black"),
                  ...
                  breaks = c("Big","Small"),
                  labels = c("Big","Small"))+
  scale_x_discrete(breaks =c("OverlapEOR_AsIs","OverlapEOR_Error","OverlapAsIs_Error"),
                   labels =c("EOR - Uncleaned", "EOR - Error", "Uncleaned - Error"))
dev.off()
```
...
habspedifferences <- do.call(rbind,lapply(split(avghabspec, avghabspec$SpeciesNum), function(num){
  out <- data.frame(SpNum = unique(num$SpeciesNum),
                    ...
                    OverlapAsIs_Error = num$habspec[num$HerbType == paste(filenames)[2]]-
                      num$habspec[num$HerbType == paste(filenames)[3]],
                    OverlapEOR_Error = num$habspec[num$HerbType == paste(filenames)[1]]-
                      num$habspec[num$HerbType == paste(filenames)[3]])
  out
  }))
...
o1 <- lm(value ~ DistAvg.N, data = overvals) # sample size of herbarium specimens
...
o6 <- lm(value ~ EOR_bg5000, data = overvals)

o7 <- lm(value ~ DistAvg.N* Background, data = overvals) # sample size of herbarium specimens
o8 <- lm(value ~ DistAvg.Dist* Background, data = overvals) # the average distance herb to eor
o9 <- lm(value ~ DistAvg.N*DistAvg.Dist , data= overvals)
...
formulas <- list(o1,o2,o3,o4,o5,o6,o7,o8,o9,o10,o11,o12)
(lmresults <- aictab(formulas,
       modnames=as.character(unlist(lapply(formulas,formula)))))
evidence(aictab(cand.set = formulas,
                modnames = as.character(unlist(lapply(formulas,formula)))))
...
evidence(aictab(cand.set = formulas,
                modnames = as.character(unlist(lapply(formulas,formula)))),
         model.low = as.character(unlist(lapply(formulas,formula)))[9])
...
ggplot(overvals, aes(DistAvg.N, value, colour=variable))+
  geom_point()+
  theme_bw()+
  stat_smooth(method = "lm")
