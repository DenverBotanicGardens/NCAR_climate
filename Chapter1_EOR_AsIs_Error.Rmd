...
```{r}
load("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt_herb_herbpluserror/habSp_EOR_uncleaned_error.Rda") #is this what i want?

# load(paste(pathstart,"habsp_bigbg.Rda", sep="")) # These don't matter, the error isn't different when the background points are different
# 
...
```{r}
# notnull are ones that have a few points but don't have a maxent file. Could pull from presence points instead??  Looks like will run into the same problem that 29 and 40 won't work like before. Not sure how I figured that out
...
# ?????
ggplot(habspe, aes(habspec, DistAvg.Dist, colour=HerbType))+
  geom_point()+
  theme_bw()+
  theme(legend.position = "none")+
  xlab("Habitat Specificity")+
...
AIC
```{r}
# have the habitat specificty as a measure of the ellispe around the 95% CI of points for each kfold replicate for each of the datasets - EOR, uncleaned, and error 
# Merged the summary information about avearge distance, min and max years with the habitat specificity, only some rows are interesting/meaningful
habspe
