...
habitatSpecificity <- function(whichones, pathstart, replicates = 10, filenames){
...
The rasternames are "Herb" and filenames
```{r}
patternmatch <- paste("PredictHerb", filenamesbig, "Sp", sep="")
...
```{r}
correlations <- function(pathstart, probmapnames, whichones, reps = 1:10){
  lapply(whichones, function(i){
  lrhist <- lapply(reps, function(k){
    r <- raster(paste(pathstart,"ProbTiffSp",i,probmapnames,k,".tif", sep=""))
