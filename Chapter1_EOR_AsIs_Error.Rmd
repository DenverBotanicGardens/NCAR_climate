...
fig3 <- 
  ...
  scale_x_discrete(limits = levels(maxevaluates$filename), breaks = NULL, labels =c("","small","","","large",""))
# 
#  c(paste(filenames[-3]), paste(filenamesbig[-3])), # limits= c("grey","white","grey30","grey","white","grey30"),
#                    labels =
...
```{r}
AUC_habsp <- merge(maxevaluates, habsp_allbg, by.x = c("filename", "SpNum"), by.y = c("HerbType","SpeciesNum"))
...
formulas <- list(o1,o2,o3,o4,o5,o7,o8,o9,o10,o12) # o6, o11,
(lmresults <- aictab(formulas,
       modnames=as.character(unlist(lapply(formulas,formula)))))
evidence(aictab(cand.set = formulas,
                modnames = as.character(unlist(lapply(formulas,formula)))))
...
evidence(aictab(cand.set = formulas,
                modnames = as.character(unlist(lapply(formulas,formula)))),
         model.low = as.character(unlist(lapply(formulas,formula)))[6])
