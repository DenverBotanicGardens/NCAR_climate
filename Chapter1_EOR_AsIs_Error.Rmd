...
accuracyhistsOneperSp <- function(pathstart, whichones = atleast12, SDorAVG = "Avg", filenames){
  lapply(whichones, function(i){
    datatype <- do.call(rbind,lapply(filenames, function(f){
    resultpath <- list.files(path = pathstart, 
                         pattern = paste(SDorAVG,"TiffSp",i,"Herb",f,
...
ggplot(habspe_aic, aes(as.factor(round(Area/(1000^2),0)), habspec, colour=HerbType))+
  geom_jitter()+
...
deal with overlap of polygons and over whole range, probably need to make changes to the function
Poop, i think I wanted ofaverage
```{r}
