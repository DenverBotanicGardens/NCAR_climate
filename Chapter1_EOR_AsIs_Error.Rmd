...
lm11 <- lm(Dist ~ habspec, data= habspe_aic) # range shift as years between herbarium and field mapped
...
o4 <- lm(value ~ Area, data= overvals)

formulas <- list(o1,o2,o3)
(lmresults <- aictab(formulas,
       modnames=as.character(unlist(lapply(formulas,formula)))))
evidence(aictab(cand.set = formulas,
                modnames = as.character(unlist(lapply(formulas,formula)))))
...
p <- ggplot(dfaccuracy, aes(value, weight = weight, colour = DataType))+
  geom_freqpoly(binwidth=0.1)+
  # geom_histogram(position = "dodge2")+
  theme_bw()+
