...
CellsOccupied <- do.call(rbind,lapply(atleast12, function(i){
  ...
  numCells <- do.call(rbind,lapply(1:10, function(k){
    ...
    spFile <- do.call(rbind,lapply(filenames[c(1:2,4)], function(f){
      ...
      out <- data.frame(CellsOccupied = cells, SampleSize = samplesize, Dataset = f, rep = k, SpNum = i, Species = g1g2namesall68$AcceptedName[i])
      out
      }))
...
ggplot(CellsOccupied, aes(SampleSize, CellsOccupied, colour = Dataset, shape = Dataset))+
...
summary(cellslm1) 
summary(lm(CellsOccupied~Dataset, tab1_cells))
ggplot(tab1_cells, aes(Dataset, CellsOccupied))+
  geom_boxplot()+
  geom_jitter()
...
ggplot(tab1_cells, aes(CellsOccupied,log(DistAvg.Dist),  colour = Dataset))+
  geom_jitter(width=.5)+
  stat_smooth(method = "lm")+
  theme_bw()
...
ggplot(habspebig, aes(y = habspec, x =log(Area), colour = HerbType))+   # (Area/(1000^2))
  geom_point()+
  stat_smooth(method="lm")+
  theme_bw()
```
...
```{r}
jpeg("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Images/Figure3_habsp_cellsoccupied.jpg",
     width=150, height=75,units='mm', res=300)

# So test by cells occupied by each dataset
ggplot(habspebig, aes(y = habspec/1000, x = log(CellsOccupied), colour = HerbType, shape=HerbType))+
  geom_jitter(height = .0001, width = 0.0001,  size=1.5)+ # shape=1,
  stat_smooth(method="lm")+
  # theme_bw()+
  ylab(expression(paste("Habitat specificity ",frac("95%CI","1000"))))+
  xlab(expression("Occupied Cells"[log]))+
  scale_color_manual(values=c("slateblue", "#E69F00", "orangered"), # c("grey50","black","white"),  # "slateblue", "#E69F00", "orangered"
                       name="Dataset",
                       breaks=c("As-Is_bg5000", "Combined_bg5000", "EOR_bg5000"),
                       labels=c("Herbarium", "Combined", "Field Survey"))+
  scale_shape_manual(values = c(3,20,2),
                     guide="none")
...
x <- splithabs[[1]]
  ...
  plots <- lapply(split(allsphabsp,
               ...
               drop=TRUE), function(x){
                      outplot <- ggplot(x, aes(Comp.1,Comp.2, colour=HerbType, shape=HerbType))+
                        geom_point()+
                        stat_ellipse()+
                        ggtitle(g1g2namesall68$AcceptedName[x$SpeciesNum[1]])+
                        # xlab("Comp.1  0.1%")+
                        theme_bw()+
                        scale_color_manual(values=c("slateblue", "#E69F00", "orangered"),  # "slateblue", "#E69F00", "orangered"
                       name="Dataset",
                       breaks=c("As-Is_bg5000", "Combined_bg5000", "EOR_bg5000"),
                       labels=c("Herbarium", "Combined", "Field Survey"))+
...
lapply(1:22, function(x){
  jpeg(paste("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Images/Supplemental_HabitatSpecificity_",x,".jpg", sep=""),
     width=150, height=75,units='mm', res=300)
...
jpeg("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Images/FigureXX_v20200305.jpg",
...
fig3 <- 
  ...
  scale_fill_manual(values = c("grey","white","grey30","grey","white","grey30"),
                    ...
                    labels = c("field survey / 500 km","herbarium / 500 km","combined / 500 km",
                               "field survey / 5 km",  "herbarium / 5 km",  "combined / 5 km"))
# +
...
# was fig 3??
ggsave(filename = "C:/Users/deprengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Uncertainty data source Chapter XX/fig4_20200319_AUC.png", fig3, width = 150, height = 80, dpi = 300, units = "mm", device='png')
...
png("C:/Users/deprengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Uncertainty data source Chapter XX/Figure5_nicheOverlap.png",
...
ggplot(overlap_values, aes(variable, value, fill = Background))+
  ...
  scale_x_discrete(breaks =c("OverlapEOR_AsIs","OverlapEOR_comb","OverlapAsIs_comb"),
                   labels =c(expression(atop("field survey -", " herbarium")), 
                             expression(atop("field survey -", " combined")), 
                             expression(atop("herbarium -", " combined"))))
...
lapply(atleast12, function(i){ 
  ...
  jpeg(paste("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Images/S1_AvgMapsSmallbg_",
       ...
       width=560, height=110,units='mm', res=300)
...
# The scale is there, just not showing up on the figures I made. Maybe they need to be wider? 
# lapply(atleast12, function(i){ 
#   rastout <- lapply(filenames[4], function(f){
#         resultpath <- list.files(path = pathstart,
#                                  pattern = paste("AvgTiffSp",i,"Herb",f,
#                                                  g1g2namesall68$AcceptedName[i], sep=""),
#                                  full.names=TRUE)
...
# For the large background extent
lapply(atleast12, function(i){ 
  
  jpeg(paste("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Images/S1_AvgMapsBigbg_",
             g1g2namesall68$AcceptedName[i],".jpg", sep=""),
       width=560, height=110,units='mm', res=300)

    rastout <- lapply(filenamesbig[-3], function(f){
     # for(f in filenames[-3]){ 
      resultpath <- list.files(path = pathstart,
                               pattern = paste("AvgTiffSp",i,"Herb",f,
                                               g1g2namesall68$AcceptedName[i], sep=""),
