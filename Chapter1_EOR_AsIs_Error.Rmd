...
CellsOccupied <- do.call(rbind,lapply(atleast12, function(i){
  numCells <- do.call(rbind,lapply(1:10, function(k){
    spFile <- do.call(rbind,lapply(filenames[c(1:2,4)], function(f){
      ...
      cells <- length(cellFromXY(rasterstack, pr[pr$kfold!=1,c("decimalLongitude.1","decimalLatitude.1")]))
...
ggplot(tab1_cells[tab1_cells$Dataset=="EOR_bg5000",], aes(Area,CellsOccupied, colour = Dataset))+
  geom_point()+
...
☺# EOR and Combined totally overlap; Herbarium covers a little more area
ggplot(tab1_cells[tab1_cells$Dataset!="As-Is_bg5000",], aes(log(Area),CellsOccupied, colour = Dataset))+
  geom_jitter(width = .2)+
  stat_smooth(method="lm")+
  theme_bw()
...
```{r}
habspebig <- merge(habsp_22species, tab1, by.x = "SpeciesNum", by.y = "SpNum1")
