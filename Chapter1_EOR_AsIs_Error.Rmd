...
for(i in which(!unlist(lapply(whichdiff_EOR2Uncleaned, is.na)))){
...
ggplot(dfaccuracy, aes(value, weight = weight/sum(weight), colour = DataType))+
  geom_density()+
  theme_bw()
```
...
ggplot(precisionsmall, aes(Precision, fill = DataType))+
  geom_histogram(position = "dodge2", bins = 10)+
  theme_bw()+
  facet_wrap(~SpNum)
