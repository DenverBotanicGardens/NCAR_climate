...
mindistxsp[[1]][[2]]@coords
seinet[seinet$scientificName == nms[1],]
...
colo <- spTransform(colo,CRS("+proj=utm +zone=13 ellps=WGS84"))
#colo row numbers to match
IDcolocnts <- data.frame(ID=c(1:length(names(colo))), Cnt=names(colo))
incounties <- over(mindistxsp[[1]][[2]],colo)

names(incounties) #row of seinet
...
seinet$pointincounty[as.numeric(names(incounties))] <- IDcolocnts$Cnt[incounties]
