...
ggplot(mndistsp, aes(year,distout,colour=genus))+
  geom_point()+
  theme_bw()

mndistsp$DistanceGroup <- cut(mndistsp$distout,
                              breaks=c(-0.0001,lseq(1,(max(mndistsp$distout))+0.000001)),
                              labels= c(1:6))
  

# Who has no order? 
table(mndistsp$order)
mndistsp[mndistsp$order == "",]

#From Plants USDA
#Add factor levels
levels(mndistsp$order)
levels(mndistsp$order) <- c(levels(mndistsp$order),"Capparales")

mndistsp$order[mndistsp$order == ""] <- c("Asterales","Capparales")

# As you get older, more distant (should be function of infomration avaliable) not consistant among Orders. 
ggplot(mndistsp, aes(DistanceGroup,year,colour=order))+
  geom_boxplot()+
  theme_bw()
```
