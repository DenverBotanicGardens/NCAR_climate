...
ggplot()+
  geom_point(data=mapdata2, aes(Easting,Northing,colour=Species))+
  theme_bw()+
  geom_polygon(data=colomap.fort.pieces, aes(x=long,y=lat,group=order,colour=id))+
#  coord_map()+
...
```{r}
#One conversion must be a bit off, points that should be in (but near state boarder) are marked as out of county

cnts <- unique(mndistsp$county)

#Maybe issues with 'map' map?
colocnts <- map("county","Colorado", fill=TRUE)
IDs <- sapply(strsplit(colocnts$names, ","), function(x) x[2])
colo <- map2SpatialPolygons(colocnts, IDs=IDs, 
                            proj4string=CRS("+proj=longlat +datum=WGS84")) 
colo <- spTransform(colo,CRS("+proj=utm +zone=13 ellps=WGS84")) #same as mindistxsp[[]][[2]]

#colo row numbers to match
IDcolocnts <- data.frame(ID=c(1:length(names(colo))), Cnt=names(colo))
seinet$IDrow <- as.numeric(row.names(seinet))

#map seinet points to match, not mindistxsp  
...
#matching row numbers for counties and herbarium specimens
incounty <- do.call(c,lapply(mindistxsp, function(sps){
  incounty <- over(sps[[2]],colo)
}))

incounty.colomap <- over(pts.all, colomap)

#names of incounty is row of seinet
#Want to do this for everything
seinet$county <- as.character(seinet$county)

seinet$pointincounty[as.numeric(names(incounty))] <- as.character(IDcolocnts$Cnt[incounty])
#try matching with another map
seinet$pointincountycolomap <- incounty.colomap$COUNTY

head(seinet[,c("county","pointincounty","pointincountycolomap")],20)
...
matched <- lapply(1:nrow(seinet), function(r){
  matchinginfo <- seinet[r,][grep(seinet$pointincounty[r],seinet[r,])]
  matchinginfo
  })

seinet[grep(seinet$pointincounty, seinet$county),]

head(seinet[is.na(seinet$pointincounty),c("locality","county","decimalLatitude","decimalLongitude")])
