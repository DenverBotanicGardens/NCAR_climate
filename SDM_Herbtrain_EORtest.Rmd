...
Maxentspecies <- lapply(7:60, function(i){
  ...
  if(is.null(checkrecords)){
    ...
    if(nrow(checkrecords)<10){
      NULL
            ...
            writeRaster(dismo::predict(meHerb, rasterstack_2),paste("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt/Herbtrain_eortest",g1g2names$AcceptedName[i],".tif", sep=""),overwrite=TRUE)
            gc()
            thres <- dismo::threshold(e) 

            out <- list(meHerb,e,thres,g1g2names$AcceptedName[i])
            out
