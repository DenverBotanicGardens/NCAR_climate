...
bg.EORs <- lapply(1:length(without41), function(x){
  out <- randomPoints(coElev, n=row(EORsxsp[[x]]@coords)+round(0.25*(nrow(EORsxsp[[x]]@coords)),0),
...
```{r}
bgherbmatch <- #match(herbpoisition,want)
  match(want,herbpoisition)
bgherbmatch <- bgherbmatch[!is.na(bgherbmatch)]

(mapply(FUN=function(x) unique(x$GNAME)[1],backgroundEORs))
(mapply(FUN=function(x) unique(x$GNAME),pca_eorandherb_SAE))
...
unique(backgroundEORs[[7]]$GNAME)

#to match by species, select the number 1:60 for EOR and herbposition[#] for herb
plot(colocounties)
points(pca_herb_SAE[[herbpoisition[54]]][,c(1:2)])
points(pca_eorandherb_SAE[[54]][,c('x1','x2')], col="red", pch=16)
points(backgroundEORs[[which(bgherbmatch==54)]][,c('x1','x2')], col="green", pch=2, cex=0.5)
