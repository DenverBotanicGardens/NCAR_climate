...
# Need only rows with lat long information

#colo.merged.cc <- colo.merged

#colo.merged.cc$decimalLatitude <- as.numeric(as.character(colo.merged.cc$decimalLatitude))
#colo.merged.cc$decimalLongitude <- as.numeric(as.character(colo.merged.cc$decimalLongitude))
...
table(seinet_TPL.cc$scientificNameAccpt)
...
reps <- 10000
rand.diff <- unlist(lapply(1:reps, function(x) median(sample(distXspbind,100))))
hist(rand.diff)
...
pointsXsp <- spTransform(distXsp[[1]][[1]]@coords, CRS("+proj=longlat +datum=WGS84"))
