...
for(i in 1:length(want)){
  ...
  Herb.thin[[i]] <- thin(loc.data = Herbsp,
                     lat.col = "decimalLatitude", long.col = "decimalLongitude",
                     spec.col="species",
                     thin.par = 1, reps=10,
                     locs.thinned.list.return = TRUE,
                     write.files = TRUE, max.files=5,
                     out.dir = "Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Thinned/",paste(out.base="Herb.thin_1km",i,"_",g1g2names$AcceptedName[wantEOR[i]],sep=""), write.log.file = TRUE,
                     log.file = "EORgridthinned_log.txt")
}

save(Herb.thin, file= "P:/hackathon/Simulations/Herb.thin.Rda")
```
...
for(i in want){
  if(!is.na(want[match(i,want)])){
    if(!is.na(wantEOR[match(i,wantEOR)])){
      plot(colocounties)
      plot(envs.backg.herb[[want[match(i,want)]]][[1]], #just the first bioclim 
         main=g1g2names$AcceptedName[want[match(i,want)]],
         add=TRUE)
      plot(envs.backg[[wantEOR[match(i,wantEOR)]]][[1]], col="grey", add=TRUE, legend=FALSE) #from the EORs
      points(pointsXsp.latlon[[i]], pch=20)
      points(distXsp[[wantEOR[match(i, wantEOR)]]][[4]]@coords, col="red", pch=16)
