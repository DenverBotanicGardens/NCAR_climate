...
coloradosps.g1g2$lon2 <- round(coloradosps.g1g2$decimalLongitude,2)
...
nrow(coloradosps.g1g2) #-
nrow(coloradosps.g1g2[!duplicated(coloradosps.g1g2[,c("lat2","lon2")]),])
...
plot(colocounties.UTM,border="gray80") #colocounties is in WGS84, not in same datum as l1G1G2
plot(l1G1G2, border="gray70",add=TRUE, lwd=7)
...
ScaleBar <- function(reference_raster_utm, round_to_nearest_km, width_percent, y_percent_from_bottom, x_percent_from_left, y_text_percent_from_bottom, ...) {
    ...
    # Horizontal percent position (x) for scale bar
        x_position <- ((xmax(reference_raster_utm)-xmin(refe???rence_raster_utm))*x_percent_from_left)+xmin(reference_raster_utm)
