...
#compare colo.test$distNearEOR with coloradosps.dist$distNearEOR
colo.test <- coloradosps.dist
colo.test$distNearEOR[which(rownames(colo.test) %in%
...
```{r}
ggplot(coloradosps.dist, aes(scientificName,distNearEOR))+
  geom_boxplot()

plot(distNearEOR~as.factor(scientificName), coloradosps.dist[coloradosps.dist$distNearEOR<500000,], las=2)

colo.merged <- merge(coloradosps.dist, g1g2names, by.x = "scientificName", by.y="AcceptedName", all.x=TRUE)

plot(distNearEOR~Family, colo.merged[colo.merged$distNearEOR<500000,], las=2)

length(table(colo.merged$Taxon)) #58 species with some data

identical(colo.test,coloradosps.dist)
