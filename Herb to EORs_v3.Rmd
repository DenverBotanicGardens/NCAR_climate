...
farawayones <- colo.all[colo.all$distNearEOR>50000 & !is.na(colo.all$distNearEOR),]
coordinates(farawayones) <-~decimalLongitude+decimalLatitude
proj4string(farawayones) <- CRS("+proj=longlat +datum=WGS84") 
farawayones <- spTransform(farawayones, CRS("+proj=utm +zone=13 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0"))

far.county <- over(farawayones, colocounties.UTM[,"COUNTY"])
farawayones$InCounty <- far.county[,1]


table(farawayones$county[farawayones$county != farawayones$InCounty])
farawayones@data[farawayones$county != farawayones$InCounty &
              !is.na(farawayones$county),c("scientificName","institutionCode","county","InCounty","georeferencedBy")]
...
plot(colocounties.UTM, border="grey80")
plot(farawayones, add=TRUE, pch=16, cex=0.5)

table(colo.all$county[colo.all$distNearEOR>50000 & !is.na(colo.all$distNearEOR)])

farawayones@data[!is.na(farawayones@data$county)&
                                              farawayones@data$county!="",]

#Make a Levenshtein distance matric between names (like that paper!)
countymatch<- adist(farawayones@data$county[!is.na(farawayones@data$county)&
                                              farawayones@data$county!=""],
                    farawayones@data$InCounty[!is.na(farawayones@data$county)&
                                              farawayones@data$county!=""],
...
farawayones$countyclean <- gsub("Co.*","",farawayones$county)
#trim trailing white space
farawayones$countyclean <- sub("\\s+$", "", farawayones$countyclean)
farawayones$countyclean <- toupper(farawayones$countyclean)

farawayones@data[!(farawayones$countyclean %in%farawayones$InCounty),]
