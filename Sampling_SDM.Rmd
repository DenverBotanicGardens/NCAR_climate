...
```{r}
# Should I include exponentials to better match maxent?
b <- c(0.5, 1, 1,
          2.5, 2.5,
          -7, -7,
          4, -4,
...
for(x in 1:10){
  ...
  jpeg(paste("C:/Users/deprengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter1/R/Fig2_NewSystematicTrue",x,".jpg", sep=""),
...
# 
# plot(xm)
# r <- response(xm, var="Elevation")
# plot(r, type = "l")
# 
# response(xm)

contribs <- xm@results[grep("contribution", dimnames(xm@results)[[1]])]
names(contribs) <- dimnames(xm@results)[[1]][grep("contribution", dimnames(xm@results)[[1]])]
```
...
```{r}
pathstart <- "C:/Users/deprengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter1/maxent_simulations_20200427/"


# 3:10, 
lapply(c(3,10, 20, 30, 50, 100), function(sampsize){
    maxent_sampling(rasterstack = rasterstack_norm, true.dist = predic_raster_quad, sampling_type = "random", reps = 10, samplesize = sampsize, pathstart = pathstart)
    
    maxent_sampling(rasterstack = rasterstack_norm, true.dist = predic_raster_quad, sampling_type = "system", reps = 10, samplesize = 100, pathstart = pathstart)
    
    maxent_sampling(rasterstack = rasterstack_norm, true.dist = predic_raster_quad, sampling_type = "convenience", reps = 10, samplesize = sampsize, pathstart = pathstart)
    
    maxent_sampling(rasterstack = rasterstack_norm, true.dist = predic_raster_quad, sampling_type = "cluster", reps = 10, samplesize = sampsize, pathstart = pathstart)
})
...
# c(3:10, 20,30,50,100) -> switch to 3, 10, 20... instead of 3:10
for(ss in c(3,10, 20,30,50,100)){
  
  jpeg(paste("C:/Users/deprengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter1/Figures/random2true_samplesize",ss,".jpg", sep=""),
       width=173, height=105,units='mm', res=300)

plot(seq(0,1,length.out = 10), seq(0,1,length.out = 10), type = "l", col="blue", lwd=1.5,
   xlab="True Distribution", ylab="Random sampling",main=paste("Suitability by Pixel, Sample Size",ss))

lapply(1:10, function(i){
  r_random <- raster(list.files(path = pathstart, 
                                    pattern = paste("ProbTiffrandomRep", i,"SampleSize",ss, ".tif", sep=""),
                                    full.names = TRUE))
  sampleof <- sample(1:length(r_random), 1000)
    # add loess local regression line
  # ls <- loess(getValues(r_random)[sampleof] ~ getValues(predic_raster_quad)[sampleof])
  # pr.loess <- predict(ls)
  # pr.loess
  model <- lm(getValues(r_random)[sampleof] ~ getValues(predic_raster_quad)[sampleof] +
                I(getValues(predic_raster_quad)[sampleof]^2))
  # myPred <- predict(model)
  # ix <- sort(getValues(predic_raster_quad)[sampleof], index.return=T)$ix
  # lines(getValues(predic_raster_quad)[sampleof][ix], myPred, col=rgb(0,1/i,1/i,0.75), lwd=1)
  # abline(lm(getValues(r_random)[sampleof] ~ getValues(predic_raster_quad)[sampleof]),
  #        col=rgb(0,1/i,1/i,0.75), lty = i)
  
  })
  
# plot(seq(0,1,length.out = 10), seq(0,1,length.out = 10), type = "l", col="red", 
#      xlab="True Distribution", ylab="Random sampling",main=paste("Suitability by Pixel, Sample Size",ss))
# # lapply(lines, function(l) lines(l~getValues(predic_raster_quad)[sampleof], col = rgb(0,0.3,0.5,0.5)))
...
for(ss in c(3:10, 20,30,50,100)){
  ...
  plot(getValues(predic_raster_quad), getValues(r_systematic), 
...
for(i in 1:5){
  r_systematic <- raster(list.files(path = pathstart, 
                                    pattern = paste("ProbTiffsystematicRep", sample(1:100,1),"SampleSize",ss, ".tif", sep=""),
                                    full.names = TRUE))
  points(getValues(predic_raster_quad), getValues(r_systematic), pch=16, cex=0.5, col = rgb(i/(i+1),0,i/(i+5),0.05))
}
  dev.off()
}
...
for(ss in c(3:10,20,30,50,100)){
  ...
  plot(getValues(predic_raster_quad), getValues(r_clust), 
...
for(i in 1:5){
  r_clust <- raster(list.files(path = pathstart, 
                                    pattern = paste("ProbTiffclusterRep",sample(1:80,1),"SampleSize",ss, ".tif", sep=""),
                                    full.names = TRUE))
  points(getValues(predic_raster_quad), getValues(r_clust), pch=16, cex=0.25, col = rgb(i/(i+1),i/(i+i),i/(i+5),0.05))
}
  dev.off()
}
...
random2true <- lapply(c(10, 20,30,50,100), function(ss){
  ...
  random2true <- data.frame(true = getValues(predic_raster_quad), random = getValues(r_random), samplesize = ss)
...
for(ss in c(3:10, 20,30,50,100)){ #
  for(sampd in c("random", "cluster", "systematic", "convenience")){
    ...
    r_design <- do.call(rbind,lapply(sample(1:100, 10), function(s){
      ...
      design2true <- data.frame(true = getValues(predic_raster_quad), V1 = getValues(r1), samplesize = ss)
...
for(i in 1:5){
  r_random <- raster(list.files(path = pathstart, 
                                    pattern = paste("ProbTiffrandomRep", sample(1:100,1),"SampleSize",ss, ".tif", sep=""),
                                    full.names = TRUE))
  points(getValues(predic_raster_quad), getValues(r_random), pch=16, cex=0.5, col = rgb(i/(i+1),0,i/(i+5),0.05))
}
  # dev.off()
# }
```
...
Cor2True <- do.call(rbind, lapply(1:(length(c(3:10, 20,30,50,100))), function(ss){
  systemcor <- do.call(rbind,lapply(1:100, function(i){
    corout <- cor(getValues(repsXsamplesize[[ss]][[i]][[1]]), getValues(predic_raster_quad), method = "pearson")
    corout
  }))
  randomcor <- do.call(rbind,lapply(1:100, function(i){
    corout <- cor(getValues(repsXsamplesize[[ss]][[i]][[2]]), getValues(predic_raster_quad), method = "pearson")
    corout
  }))
  conveniencecor <- do.call(rbind,lapply(1:100, function(i){
    corout <- cor(getValues(repsXsamplesize[[ss]][[i]][[3]]), getValues(predic_raster_quad), method = "pearson")
    corout
  }))
  clustercor <- do.call(rbind,lapply(1:100, function(i){
    corout <- cor(getValues(repsXsamplesize[[ss]][[i]][[4]]), getValues(predic_raster_quad), method = "pearson")
    corout
  }))
