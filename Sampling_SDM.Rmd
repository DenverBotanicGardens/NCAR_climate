...
# linear predictor 
lm1 <- as.formula(~ -1 + getValues(rasterstack_norm[[1]])*1 + getValues(rasterstack_norm[[2]])*2.5 -
                    getValues(rasterstack_norm[[3]])*6 + getValues(rasterstack_norm[[4]])*4 -
                    getValues(rasterstack_norm[[5]])*5 + getValues(rasterstack_norm[[6]])*.005)
...
# The lapply must be pulling things from the global environment and not the repeated sampling of occurrences. 
# paramsout_sim <- lapply(1:2, function(x){
#   print(paste("Rep",x))
#   # generate presence-absence data
#   y <- predic_raster
#   y <- setValues(y, as.vector(rbinom(length(y), 1, getValues(predic_raster))))
#   plot(y)
#     # or y_1 is the sample from the sampling alogorithm
#   # keep resence-only data
#   y_1 <- do.call(cbind,lapply(1:6, function(i){
#     rasterstack_norm[[i]][y==1]
#     }))
...
save(paramsout_sim, file = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter1/R/Fig2_NewSystematicTrue1.jpg")
...
datapsi <- exp(beta0 + beta1*rasterstack_norm[[1]][y == 1] + beta2*rasterstack_norm[[1]][y == 1]^2 + 
                   beta3*rasterstack_norm[[2]][y == 1] + beta4*(rasterstack_norm[[2]][y == 1]^2) -
                   beta5*rasterstack_norm[[3]][y == 1] - beta6*(rasterstack_norm[[3]][y == 1]^2) + 
                   beta7*rasterstack_norm[[4]][y == 1] + beta8*(rasterstack_norm[[4]][y == 1]^2) -
                   beta9*rasterstack_norm[[5]][y == 1] - beta10*(rasterstack_norm[[5]][y == 1]^2) + 
                   beta11*rasterstack_norm[[6]][y == 1] + beta12*(rasterstack_norm[[6]][y == 1]^2))/
    (1 + exp(beta0 + beta1*rasterstack_norm[[1]][y == 1] + beta2*rasterstack_norm[[1]][y == 1]^2 + 
                   beta3*rasterstack_norm[[2]][y == 1] + beta4*(rasterstack_norm[[2]][y == 1]^2) -
                   beta5*rasterstack_norm[[3]][y == 1] - beta6*(rasterstack_norm[[3]][y == 1]^2) + 
                   beta7*rasterstack_norm[[4]][y == 1] + beta8*(rasterstack_norm[[4]][y == 1]^2) -
                   beta9*rasterstack_norm[[5]][y == 1] - beta10*(rasterstack_norm[[5]][y == 1]^2) + 
...
for(x in 1:10){
  samplesys1 <- systematic_sampling(predic_raster, samplesize = 100, studyarea = 100)
  ...
  plot(predic_raster, main = ""); points(samplesys1[[1]], pch=16, cex=0.25)
...
maxent_sampling <- function(rasterstack, true.dist, sampling_type = c("random","systematic","convenience","cluster"),
  ...
  if(grepl(sampling_type, "systematic")){
    sample_xy <- systematic_sampling(suitability.raster = true.dist, samplesize = samplesize, studyarea = 100)
  }
...
# Failed on 3/26/2020 at sample size 6, why?!, maybe all 50% suitable?
lapply(c(6:10, 20, 30, 50, 100), function(sampsize){
...
pathstart1 <- "C:/Users/deprengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter1/maxent_simulations_20200330/"

lapply(c(3:10,20,30,50,100), function(sampsize){
  maxent_sampling(rasterstack = rasterstack_norm, true.dist = predic_raster, sampling_type = "systematic", reps = 100,samplesize = sampsize, 
                  pathstart = pathstart1)
})
