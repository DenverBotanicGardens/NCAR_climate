...
grViz("digraph flowchart {
      ...
      [8]: c('Adaptive cluster','Convenience','Simple random')
      ")
...
clru1_z <- generateSpFromFun(raster.stack = rasterstack3_z, parameters = clru.para_z,
                             species.type = "mixed",
                             formula = "bio1_12 + bio12_12 + (bio11_12 * bio19_12) + ElevationResampled_bioclim + dummy", 
...
surroundingcells <- function(cellRowCol){
  out <- data.frame(Cells = c(cellFromRowCol(suitability.raster, cellRowCol[1]-1, cellRowCol[2]-1),
                          cellFromRowCol(suitability.raster, cellRowCol[1]-1, cellRowCol[2]),
                          cellFromRowCol(suitability.raster, cellRowCol[1]-1, cellRowCol[2]+1), # row above
                          cellFromRowCol(suitability.raster, cellRowCol[1], cellRowCol[2]-1),
                          cellFromRowCol(suitability.raster, cellRowCol[1], cellRowCol[2]+1), # either side, same row
                          cellFromRowCol(suitability.raster, cellRowCol[1]+1, cellRowCol[2]-1),
                          cellFromRowCol(suitability.raster, cellRowCol[1]+1, cellRowCol[2]),
                          cellFromRowCol(suitability.raster, cellRowCol[1]+1, cellRowCol[2]+1)),
                    Rows = c(cellRowCol[1]-1,
                          cellRowCol[1]-1,
  ...
  out
        ...
        # Instead of the first if statement, start with the cluster function that picks a random point, builds a cluster until no additional cells are added, then if the length of the cluster is smaller than the sample size, run the cluster again.  
      # the length of sampleout is the sample size, can take rows up to sample size
cluster <- function(cells){
  randomcell <- c(ceiling(runif(1, min = 1, max = nrow(suitability.raster))),
               ceiling(runif(1, min = 1, max = ncol(suitability.raster))))
  out <- lapply(1:8, function(c){
    if(rbinom(1,1,getValues(suitability.raster)[cells[c,1]])==1){
      samples <- rbind(sampleout,xyFromCell(suitability.raster, cells[c,1]))
      additionalsurrounding <- surroundingcells(as.numeric(paste(cells[c,c(2:3)])))
      samples_cells <- list(samples, additionalsurrounding)
      samples_cells
    }
    })
  samples <- unlist(out, recursive = FALSE)
  samples
} # end clustser function

adaptive_cluster <- function(suitability.raster, samplesize){

  i <- 1
  while(i <= samplesize){
    if(rbinom(1,1,suitability.raster[randomcell[1],randomcell[2]])==1){
      sampleout <- rbind(sampleout,xyFromCell(suitability.raster, cellFromRowCol(suitability.raster, randomcell[1], randomcell[2])))
      i <- i+1
      surcells <- surroundingcells(randomcell)
