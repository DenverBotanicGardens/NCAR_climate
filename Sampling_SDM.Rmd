...
plot(1:10, 1:10, type="l", lwd=2, yaxt="n", xaxt="n", ylab="Biased Sampling", xlab="True distribution")
# polygon(c(1,1,8.9), c(2.1,9.9,9.9), col=rgb(.15,0,.5,.25))
arrows(5,5.1,2.5,7.5,lwd=1, col='red')
arrows(5,4.9,7.5,2.5, col='red')
...
for(x in 1:10){
  ...
  plot(predic_raster, main = "Systematic sampling (n = 100)"); points(samplesys1, pch=16, cex=0.5)
  }
...
for(x in 1:10){
  ...
  plot(predic_raster, main = "Simple random sampling (n = 100)"); points(random1, pch=16, cex=0.5)
  mtext
  }
...
for(x in 1:10){
  ...
  print(nrow(cluster1))
  plot(predic_raster, main = "Adaptive cluster sampling (n = 100)"); points(cluster1, pch=16, cex=0.25)
}   
