...
systematic_sampling <- function(suitability.raster,  samplesize){
  ...
  randcell <- cellFromRowCol(suitability.raster,randcellRowCol[1],randcellRowCol[2]) # cell number that matches
    # systematically sample the study area
  evenstep <- ceiling(studyarea/(samplesize*100))
  i <- evenstep
  
  sampleout <- c()
  # set the 'study area' of 50 x 50 cells so study area == 50
      # Systematically sample and then use thinmax to get the samplesize
      studyarea <- function(cellRowCol, suitability.raster, samplesize, studyarea){
        # if(all(cellRowCol > 20))
        if(cellRowCol[1] > (nrow(suitability.raster)-(studyarea+1))) cellRowCol[1] <- (nrow(suitability.raster))-(studyarea+1)
        if(cellRowCol[2] > (ncol(suitability.raster)-(studyarea+1))) cellRowCol[2] <- (ncol(suitability.raster))-(studyarea+1)
        # steps <- ceiling((round(sqrt(studyarea),0))/(round(sqrt(samplesize),0)))*10 # need at least as many to evenly space out samples
        out <- data.frame(Cells = c(cellFromRowCol(suitability.raster, cellRowCol[1], cellRowCol[2]), # top row, min(col) to max(col)
