...
## UPDATE WITH SAMPLE SIZE IN NAME!!
for(ss in c(3:10,20)){
  
  r_random <- raster(list.files(path = pathstart, 
                                    pattern = paste("ProbTiffrandomRep", sample(1:100,1),"SampleSize",ss, ".tif", sep=""),
                                    full.names = TRUE))
  plot(getValues(predic_raster), getValues(r_random), pch=16, cex=0.5)
for(i in 1:5){
  r_random <- raster(list.files(path = pathstart, 
                                    pattern = paste("ProbTiffrandomRep", sample(1:10,1), ".tif", sep=""),
                                    full.names = TRUE))
  points(getValues(predic_raster), getValues(r_random), pch=16, cex=0.5, col = i)
  }
}
...
# plot(predic_raster)
    ...
    r_clust <- raster(list.files(path = pathstart, 
                                    ...
                                    full.names = TRUE))
  plot(getValues(predic_raster), getValues(r_clust), pch=16, cex=0.25)
for(i in 1:5){
  r_random <- raster(list.files(path = pathstart, 
                                    pattern = paste("ProbTiffclusterRep", sample(1:10,1), ".tif", sep=""),
                                    full.names = TRUE))
  points(getValues(predic_raster), getValues(r_clust), pch=16, cex=0.25, col = i)
  }
