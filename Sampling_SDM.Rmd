...
for(alpha in seq(1,10,length.out = 10)){
  for(beta_1 in seq(1,10,length.out = 10)){
...
writeRaster(rasterstack_norm, "C:/Users/deprengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter1/rasterstack_normalized.tif", overwrite=TRUE)
```
lognormal
alpha1 <- log(mu)-(1/2)*log((sig2+mu^2)/mu^2)
beta1 <- sqrt(log((sig2+mu^2)/mu^2))
...
lm1 <- as.formula("getValues(rasterstack_norm[[1]])*1 + getValues(rasterstack_norm[[2]])*2 + getValues(rasterstack_norm[[3]])*3 +
                    getValues(rasterstack_norm[[4]])*4 + getValues(rasterstack_norm[[5]])*5 + getValues(rasterstack_norm[[6]])*.01")

predic_raster[] <- exp(as.formula(lm1[[2]]))/(1+exp(as.formula(lm1[[2]])))

# predic_raster[] <- exp((getValues(rasterstack3_z[[1]])^2)*1 + log(getValues(rasterstack3_z[[2]]))*2 + getValues(rasterstack3_z[[3]])*3 + getValues(rasterstack3_z[[4]])*4 + getValues(rasterstack3_z[[5]])*0.001)/
#   (1-exp((getValues(rasterstack3_z[[1]])^2)*1 + log(getValues(rasterstack3_z[[2]]))*2 + getValues(rasterstack3_z[[3]])*3 + getValues(rasterstack3_z[[4]])*4 + getValues(rasterstack3_z[[5]])*0.001))
...
# should result in 0-1 but doesn't
predic_raster[] <- exp(getValues(rasterstack3_z[[1]])*1 + 
                         # lognormal
                         (1/(getValues(rasterstack3_z[[2]])*sqrt(2*pi*beta1^2)))*
                         (exp(-(log(getValues(rasterstack3_z[[2]]))-alpha1)^2)/(2*beta1^2)) + 
                         # linear?
                         getValues(rasterstack3_z[[3]])*3 + getValues(rasterstack3_z[[4]])*4 +
                         getValues(rasterstack3_z[[5]])*1)/
                    (1-exp(getValues(rasterstack3_z[[1]])*1 + 
                             # lognormal
                             (1/(getValues(rasterstack3_z[[2]])*sqrt(2*pi*beta1^2)))*
                             (exp(-(log(getValues(rasterstack3_z[[2]]))-alpha1)^2)/(2*beta1^2)) + 
                             # linear?
