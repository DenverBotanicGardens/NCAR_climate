...
MODIS hdf files   
```{r}
# detailed data on hdf4 files
# gdalinfo("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_GENERAL_non-species_files/All_General_Background_Layers/Colorado/MODIS_monthlysnowcover/72159724/MOD10CM.A2011001.006.2016053180423.hdf")

products <- mt_products()
# MOD13Q1	MODIS/Terra Vegetation Indices (NDVI/EVI) 16-Day L3 Global 250m SIN Grid

# via MODISTools
df<- data.frame("site_name" = "COLO")
df$lat <- 40
df$lon <- -109

subsets <- mt_batch_subset()

dates <- mt_dates(product = "MOD10CM", lat = 40, lon = -109)

snowpath <- "Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_GENERAL_non-species_files/All_General_Background_Layers/Colorado/MODIS_monthlysnowcover/"

# Have to run next two at same time, otherwise it drops the WD
# setwd("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_GENERAL_non-species_files/All_General_Background_Layers/Colorado/MODIS_monthlysnowcover/")
# folders <- list.files("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_GENERAL_non-species_files/All_General_Background_Layers/Colorado/MODIS_monthlysnowcover/")
# end with $
files <- dir(path = snowpath, pattern = ".hdf$", recursive = TRUE, full.names = TRUE)

# sds <- get_subdatasets("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_GENERAL_non-species_files/All_General_Background_Layers/Colorado/MODIS_monthlysnowcover/72159724/MOD10CM.A2011001.006.2016053180423.hdf")
sds <- sapply(files, function(x){ 
  sds1 <- get_subdatasets(x)
  sds1[1]
  })

# Where is the month information?!!?
YearMonth <- sapply(sds, function(s) {
  out <- strsplit(s, "MOD10CM.A")
  substr(out[[1]][2],1,4)
  })
lapply(1:length(sds), function(i) gdal_translate(sds[i], dst_dataset = paste(snowpath, "Snow_Cover", YearMonth[i],"_",i, ".tif", sep="")))


resultpath <- list.files(path = snowpath,
                           pattern = ".tif",
                           full.names=TRUE)
...
What about Bio12 and Bio19 - they look super identical
```{r}
...
# which have smallest distance? 
intrinsicdisc[order(abs(intrinsicdisc$IntrinsicDisc)),]
```
...
HellingerDist[order(abs(HellingerDist$IHDist)),]
...
for(r in 1:reps){
  ...
  xm <- dismo::maxent(x = rasterstack, p = sample_xy, args=c(
    ...
    'defaultprevalence=0.5', # to match the virtual plants' alpha, intercept term. and that it's hard to measure with small numbers of points
    'betamultiplier=0',
    # 'pictures=true',
    # 'randomtestpoints=30',
    'linear=true',
...
pathstart <- "C:/Users/deprengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter1/maxent_simulations_20200615/"
...
# Pattern: correlation to the true distribution  ***Use the all one plot below, next****
```{r}
...
```
# Pattern: correlation to the true distribution  All one plot; no bio1
```{r}
...
p
 ...
 ggsave("C:/Users/deprengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter1/Figures/AlldesignsAllss_noBio1.jpg",
        grid.arrange(plots_random[[1]], plots_random[[2]],plots_random[[3]],
                     plots_random[[4]],plots_random[[5]],plots_random[[6]],
                     plots_conven[[1]], plots_conven[[2]],plots_conven[[3]],
                     plots_conven[[4]],plots_conven[[5]],plots_conven[[6]],
...
load(modelstoload[1])
xm@lambdas # the weights? Why are there three? the first is the "variable coefficient, then the min and max of that variable for the inut data. If the coefficient is 0, that variable was not included in the model. "
class(xm)
