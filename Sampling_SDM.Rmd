...
for(ss in c(3:10, 20,30,50,100)){
  for(sampd in c("random", "cluster", "systematic", "convenience")){
    ...
    r_random <- raster(list.files(path = pathstart, 
                                      pattern = paste("ProbTiff", sampd, "Rep", sample(1:100,1),"SampleSize",ss, ".tif", sep=""),
                                      full.names = TRUE))
  
    random2true <- data.frame(true = getValues(predic_raster), V1 = getValues(r_random), samplesize = ss)
    # names(random2true)[names(random2true == "V1")] <- sampd

    p <- ggplot(random2true, aes(true, V1, colour = as.factor(samplesize)))+
...
Try to grob all onto one plot
```{r}
pp1 <- ggplot_build(random2true[[1]][[2]])
pp2 <- ggplot_build(random2true[[2]][[2]])$data[[1]]
pp1$data[[1]]$fill[grep(pattern = "^2", pp2$group)] <- pp2$fill[grep(pattern = "^2", pp2$group)]

# build plot grobs
grob1 <- ggplot_gtable(pp1)
# grob2 <- ggplotGrob(random2true[[2]][[2]])
grob2 <- list()
for(i in 2:5){
  grob2[[i]] <- ggplotGrob(random2true[[i]][[2]])
}
...
leg2 <- list()
for(i in 2:5){
  leg2[[i]] <- gtable_filter(grob2[[i]], "guide-box")
}
leg <- gtable:::rbind_gtable(leg1[["grobs"]][[1]],  leg2[[2]][["grobs"]][[1]],
                             # leg2[[3]][["grobs"]][[1]],
                             # leg2[[4]][["grobs"]][[1]],
                             # leg2[[5]][["grobs"]][[1]],
                             "first")
leg <- gtable:::rbind_gtable(leg, leg2[[3]][["grobs"]][[1]], "first")
leg <- gtable:::rbind_gtable(leg, leg2[[4]][["grobs"]][[1]], "first")
leg <- gtable:::rbind_gtable(leg, leg2[[5]][["grobs"]][[1]], "first")
