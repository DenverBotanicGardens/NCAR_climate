...
toVistrails1 <- do.call(rbind,(lapply(1:14, function(x){ 
  data.frame(pca_data4[[x]],SpeciesID = x)
  })
))

toVistrails2 <- do.call(rbind,(lapply(15:28, function(x){ 
  data.frame(pca_data4[[x]],SpeciesID = x)
  })
))
# 
# toVistrails3 <- do.call(rbind,(lapply(15:21, function(x){ 
#   data.frame(pca_data4[[x]],SpeciesID = x)
#   })
# ))
# 
# toVistrails4 <- do.call(rbind,(lapply(22:28, function(x){ 
#   data.frame(pca_data4[[x]],SpeciesID = x)
#   })
# ))
...
write.csv(toVistrails1, "Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/VisTrails/FieldData1_EORG1G2_WGS84.csv")

write.csv(toVistrails2, "Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/VisTrails/FieldData2_EORG1G2_WGS84.csv")
```
...
rasterstack_2 <- brick(list(coElev, coAspect, coSlope))
...
save(pca_data4, file= "Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Data/pca_data4.Rda")


pca_data4[[1]][pca_data4[[1]]$pa==3,]
length(pca_data4) ##28 WHOO HOOOO!
```

Could do it in 4 sets, two on desktop, two on laptop
```{r}
  i<-pca_data4[[2]]
  ...
  checksp <- sapply(split(x, x$pa), function(y) nrow(y))
    ...
    if(sum(checksp<10)==0){
        for(l in 1:10){
          speciestable <- x
          kfoldnum<-4
          speciestable$group_pa <- unlist(lapply(split(speciestable, speciestable$pa), function(sp){
            kfold(sp$pa, kfoldnum)
          }))

lapply(split(speciestable, speciestable$pa), function(sp){
           length(kfold(sp$pa, kfoldnum))
          })
          ...
          meEOR <- maxent(rasterstack_2, p=train_pEOR, a=train_bEOR,
                          path = paste("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt/",unique(i$GNAME)[1],l,"_EOR",sep=""))
          meHerb <- maxent(rasterstack_2, p=train_pHerb, a=train_bHerb,
                          path = paste("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt/",unique(i$GNAME)[1],l,"_herb",sep=""))
          ...
          writeRaster(dismo::predict(meHerb,rasterstack_2),
                      paste("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt/HerbOnly_",unique(i$GNAME)[1],l,".tif", sep=""),overwrite=TRUE)
          gc()
          writeRaster(dismo::predict(meEOR, rasterstack_2),paste("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt/EOROnly_",unique(i$GNAME)[1],l,".tif", sep=""),overwrite=TRUE)
          gc()

          thresEOR <- dismo::threshold(eEOR)
          thresHerb <- dismo::threshold(eHerb)

          out[[l]] <- list(unique(i$GNAME)[1],
...
rows <- 1:nrow(rasterstack_2)
split <- sort(rows%%ncores)+1

#Loop through all the species
MaxentspeciesEORandHerbforeach <- lapply(pca_data4, function(i_sp){
...
lapply(split(speciestable, speciestable$pa), function(sp){
          ...
          meEOR <- maxent(rasterstack_2, p=train_pEOR, a=train_bEOR,
                          path = paste("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt/",unique(i_sp$GNAME)[1],l,"_EOR",sep=""))
          meHerb <- maxent(rasterstack_2, p=train_pHerb, a=train_bHerb,
                          path = paste("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt/",unique(i_sp$GNAME)[1],l,"_herb",sep=""))
          ...
          prediction <- foreach(i=unique(split), .export=c("rows"), .packages = c("dismo"), .combine=c)%dopar%{
              rows_sub <- rows[split==i]
              sub <- raster::crop(rasterstack_2,raster::extent(rasterstack_2, min(rows_sub), max(rows_sub), 1, ncol(rasterstack_2)))
          
              
              #Visulatize predictions
              writeRaster(dismo::predict(sub, meHerb),
                          paste("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt/HerbOnly_",unique(i_sp$GNAME)[1],l,".tif", sep=""),overwrite=TRUE)
              gc()
              writeRaster(dismo::predict(meEOR, rasterstack_2),paste("Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/RMaxEnt/EOROnly_",i,unique(i_sp$GNAME)[1],l,".tif", sep=""),overwrite=TRUE)
              gc()
          }
          ...
          out[[l]] <- list(unique(i_sp$GNAME)[1],
                      meEOR,eEOR,thresEOR,eEOR@auc,
                      meHerb,eHerb,thresHerb,eHerb@auc)
...
save(MaxentspeciesEORandHerbforeach, file= "Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Data/MaxentspeciesEORandHerbforeach.Rda")
