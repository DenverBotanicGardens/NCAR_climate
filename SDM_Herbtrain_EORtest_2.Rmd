...
forlength60 <- intersect(which(!is.na(herbposition)),which(!is.na(MEposition)))
length(forlength60) # 28

forlength59 <- intersect(pca_eorposition[!is.na(pca_eorposition)],which(!is.na(MEposition)))
length(forlength59)

forlengthbackgroundeor <- intersect(1:59, which(!is.na(MEposition)))


pca_data3 <- lapply(1:60, function(x){
  names(pca_herb_SAE[[forlength60[x]]]) <- names(pca_eorandherb_SAE[[forlength59[x]]])
  pca_eorandherb_SAE[[forlength59[x]]]$pa <- 1
  bg.herb_SAE[[forlength60[x]]]$pa <- 4
  backgroundEORs[[forlength59[x]]]$pa <- 3
  pca_herb_SAE[[forlength60[x]]]$pa <- 2
  rbind(pca_eorandherb_SAE[[forlength59[x]]],
        bg.herb_SAE[[forlength60[x]]],
        backgroundEORs[[forlength59[x]]],
        pca_herb_SAE[[forlength60[x]]])
})
