...
f7 <- lm(distNearEOR ~ New.Genus, data= colo.all.ll)
...
# HerbAreasDist <- mapply(function(x,y,z){ 
#   zout <- z@data
#   data.frame(z,Dist=y,Area=x)
#   }, 
...
HAD_all$InstNear[HAD_all$z.institutionCode %in% local] <- "Local"
HAD_all$InstNear[HAD_all$z.institutionCode %in% Adjacent] <- "Near"
HAD_all$InstNear[HAD_all$z.institutionCode %in% far] <- "Far"

HAD_all2 <- merge(g1g2names,HAD_all,by.x="Taxon", by.y="z.scientificName", all.y=TRUE)

nrow(HAD_all);nrow(HAD_all2) #Whew!

HAD_all2$z.year

nrow(HAD_all2[HAD_all2$z.year>1800,]);nrow(HAD_all);nrow(HAD_all2) 
table(HAD_all2$z.year)
table(HAD_all2$InstNear)
str(HAD_all2$InstNear)
```

```{r}
f1 <- lm(y ~ as.factor(New.Genus) + z.year + as.factor(InstNear) + as.factor(Family) + x, data= HAD_all2)
f2 <- lm(y ~ z.year + as.factor(InstNear) + as.factor(Family) + x, data= HAD_all2)
f3 <- lm(y ~ z.year + x, data= HAD_all2)
f4 <- lm(y ~ z.year, data= HAD_all2)
f5 <- lm(y ~ x, data= HAD_all2)
f6 <- lm(y ~ 1, data= HAD_all2)
f7 <- lm(y ~ as.factor(New.Genus), data= HAD_all2)
...
table(HAD_all2$z.institutionCode)
```
...
ggplot(HAD_all2[HAD_all2$y<500000&HAD_all2$z.year>1800,], aes(InstNear, y/1000))+
  geom_point()+
  theme_bw()+
  stat_smooth(method="lm")+
  ylab("Distance (km) to nearest EOR")+
