  ...
  #put grid points across colorado, sample from it for each polygons
     ...
     colorado.grid <- lapply(1:length(polys), function(i){
       grid <- lapply(1:length(polys), function(i){
         if(sqrt((polys[i,]@bbox[3]-polys[i,]@bbox[1])*(polys[i,]@bbox[4]-polys[i,]@bbox[2]))/1000 < 1){
         ...
         grid <- makegrid(polys[i,], cellsize = 500, pretty=FALSE) #so maybe every 800 meters?, even though scale is 1km or bigger!
         if(nrow(grid)>0){
           gridout <- SpatialPointsDataFrame(grid, data.frame(id=1:nrow(grid),
                                                            polys@data$PolyID@data[i,]),
                                           proj4string = CRS(proj4string(polys[i,])))
     ...
     #before extracting bioclim data, project grid points to 
     #  > paste(rscrs)
...
plot(colocounties.UTM)
plot(colorado.grid[[1]][[1]], add=TRUE)
plot(polys[1,], border="blue", add=TRUE, lwd=10)

for(i in 1:length(polys)){
  plot(polys[i,], border="blue")
  plot(colorado.grid[[i]][[i]], pch=20, cex=0.5, add=TRUE)
  ScaleBar(polys[i,],
