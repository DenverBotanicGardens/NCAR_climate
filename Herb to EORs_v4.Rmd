...
coloradosps.dist$distNearEOR <- NA
    ...
    utm_crs <- CRS("+proj=utm +zone=13 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0")
latlong_crs <- CRS("+proj=longlat +datum=NAD83 +no_defs +ellps=GRS80 +towgs84=0,0,0")

proj4string(l1G1G2)<-utm_crs
```
...
distXsp <- lapply(1:nrow(g1g2names), function(i){
  ...
  if(nrow(EORpnts)>0){
    ...
    EORpnts <- spTransform(EORpnts, latlong_crs) # CRS("+proj=longlat +datum=WGS84"))
  } else {
    EORpnts <- NA
  }
  ...
  if(nrow(g1g2now)>0){
    ...
    distnow <- apply(gDistance(g1g2now,
                     ...
                     2,min)
    g1g2now$SpeciesID <- i
    g1g2now$dist <- distnow
    g1g2now$area <- sum(areanow)
    g1g2now$numpolys <- length(areanow)
  ...
  save(distXsp, file= "Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Data/distXsp1.Rda")
```

```{r}
str(coloradosps.dist)
distXsp1 <- do.call(c,mapply('[[', distXsp, 1))
...
pca_eorandherb <- lapply(1:nrow(g1g2names), function(sp){
  #put grid points across colorado, sample from it for each polygons
     ...
     data_eor <- data.frame(coordinates(mergedEORs),id=mergedEORs$id,polyid=mergedEORs$polyid,
                            ...
                            Lastobs=mergedEORs$LASTOBS,dist=NA,area=NA,
