...
# library(ggsn)
# proj4string(l1G1G2) <- CRS(proj4string(colocounties.UTM))
# coordinates(colocounties.UTM)
# polygons(l1G1G2)
# 
# 
# ggplot(l1G1G2[l1G1G2$GNAME %in% c(g1g2names$AcceptedName[1],g1g2names$Taxon[1]),])+
#   aes([,1],[,2], group=ID)+
#   geom_polygon()+
#   geom_path(color="black")+
#   scale_fill_brewer("EOR")
```
...
hist(distXspbind[distXspbind<700000]/1000, main = "Distance (km) from points to nearest EOR",
     xlab="km", breaks=50)#, xlim=c(1,612))

plot(density(distXspbind/1000))
...
ggplot(toplot[toplot$distXspbind<max(toplot$distXspbind)&
                toplot$area_distxsp<max(toplot$area_distxsp),], aes(area_distxsp, distXspbind/1000))+
  geom_point()+
  theme_bw()+
...
Distance from herbarium to EOR by year  
```{r}
#list(g1g2now, distnow, areanow, EORpnts)
#distxsp list
...
ggplot(HAD_all[HAD_all$y<500000&HAD_all$z.year>1800,], aes(z.year, y/1000))+
  geom_point()+
  stat_smooth(method="lm")+
  theme_bw()+
  xlab("Year")+
...
ggplot(HAD_all[HAD_all$y<500000&HAD_all$z.year>1800,], aes(x/1000000, y/1000))+
  geom_point()+
  theme_bw()+
  stat_smooth(method="lm")+
  ylab("Distance (km) to nearest EOR")+
...
ggplot(HAD_all2[which(HAD_all2$y<500000&HAD_all2$z.year>1800&
                  !is.na(HAD_all2$New.Genus)),], aes(z.year, y/1000, colour=as.numeric(New.Species)))+
  geom_point()+
  theme_bw()+
  ylim(c(-1,max(HAD_all2$y[which(HAD_all2$y<500000&HAD_all2$z.year>1800&
                  !is.na(HAD_all2$New.Genus))]/1000)+1))+
  stat_smooth(method="lm")+
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  scale_color_gradientn(colours = rainbow(length( table(HAD_all2$Taxon))))
...
```{r}
f1 <- lm(y ~ as.factor(New.Genus)*z.year + x, data= HAD_all2)
f2 <- lm(y ~ z.year + x, data= HAD_all2)
f3 <- lm(y ~ z.year, data= HAD_all2)
f4 <- lm(y ~ x, data= HAD_all2) #x is the area of the species known range
f5 <- lm(y ~ 1, data= HAD_all2)
f6 <- lm(y ~ z.year*x, data= HAD_all2) #year given the range
f7 <- lm(y ~ as.factor(New.Genus)*(z.year), data= HAD_all2)
...
length(table(HAD_all2$New.Genus))
...
pca_all <- lapply(1:length(pca_data2), function(i){
  x <- animatepca_data[[1]]
  x.pca <- princomp(x[,c("WinterPrecip","SummerPrecip","WinterMinTemp","WinterMaxTemp","FallMaxTemp")], cor=TRUE)
  # x.load <- loadings(x.pca)
  x.pc1 <- predict(x.pca)
  x.pc <- data.frame(x.pc1,x) #x$pa where 1=EOR presence, 3=EOR background; 2=Herb presence, 4=herb background
...
# pca_all[[1]][[1]]
# 
# what <- unlist(pca_all, recursive = FALSE)
# animate_all <- do.call(rbind, lapply(pca_all, function(i){
#   x.pc <- sapply(i,'[[',1)
#   x.pc
#   }))
...
animatePCA <- ggplot(x.pc, aes(Comp.1, Comp.2, 
                               colour=as.factor(pa), 
                               shape=as.factor(pa),
                               frame=SpeciesID))+
                  geom_point()+
                  stat_ellipse()+
                  theme_bw()+
                  xlab(paste("PC1 (",round(x.PoV[1],2)*100,"%)",sep=""))+
                  ylab(paste("PC2 (",round(x.PoV[2],2)*100,"%)",sep=""))+
                  theme(plot.title=element_text(hjust=0))+
                  theme(legend.justification=c(0,1), legend.position=c(0,1),
                        legend.background = element_rect(fill=alpha('white', 0.1)))+
                  labs(color = "Presence/Background", shape = "Presence/Background")
                  # scale_color_manual("Presence/Background",
                  #                    labels = c("EOR pr","Herb pr","EOR bg","Herb bg"),
                  #                    values = c("darkorchid1","coral","darkorchid4","chocolate1"))+
                  # scale_shape_manual("Presence/Background", values=c(16,4,16,4))

gganimate(animatePCA, "Q:/Research/Projects/alpine-phenology/Uncertainty in niche modeling/Images/PCA_animate.gif", ani.width = 600, ani.height = 450, interval = 0.75)
