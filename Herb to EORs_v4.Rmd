...
pca_eorandherb <- lapply(1:nrow(g1g2names), function(i){
  #put grid points across colorado, sample from it for each polygons
     polys <- l1G1G2[l1G1G2$GNAME %in% c(g1g2names$AcceptedName[i],
                                         g1g2names$Taxon[i]),] 
     #CRS("+proj=utm +zone=13 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0") to rscrs ('raster stack CRS')   The bioclim data is in 
     ##But to make a grid, need it in UTM, convert later?!?!
     #polys <- spTransform(polys, CRS(paste(rscrs)))
  split.polys <- disaggregate(polys)
     #str(split.polys@data$PolyID@data)
     #str(split.polys@polygons)
     #plot(split.polys[1,])
     #Data associated with each polygon
     #str((polys@data$PolyID@data))
     
     #Could make Colorado grid and then select point where polygons overlap, but still want to split by polygon, or at least get data from each one
  colorado.grid <- makegrid(colocounties.UTM, cellsize = 1000)  
```

```{r}
#Problems because NAs not permitted in row index. Likey because of subsetting the polygons there are uneven removals throughout the data structure so NAs are introducted. Following soemone's function to get rid of NAs in sp dataframe object: x is a spatial dataframe object, margin to remove rows (1) or columns (2)
...
polys2 <- sp.na.omit(polys)     
  dim(polys)
    dim(polys2) #dim is now 0 and 41 down from 151? eeek, that's not right

# PLOT DELETED POINTS IN RED    
plot(polys, col="red", pch=20)
  plot(polys2, col="black", pch=20, add=TRUE)
```

```{r}
  
  colorado.grid <- lapply(1:length(polys), function(i){
    out <- SpatialPointsDataFrame(colorado.grid, data.frame(id=1:nrow(colorado.grid),
                                                            polys[i,]@data$PolyID@data),
                                         proj4string = CRS(proj4string(polys)))
    row.names(out)
    ov <- sp::over(out, polys[1,])
  })
  
     grid <- lapply(1:length(polys), function(i){
       grid <- makegrid(split.polys[i,], cellsize = 10) #so maybe every 100 meters?
       gridout <- SpatialPointsDataFrame(grid, data.frame(id=1:nrow(grid),
                                                          split.polys@data$PolyID@data[i,]),
                                         proj4string = CRS(proj4string(split.polys)))
       gridout
       })
     #All for one species right now
     data_EOR <- data.frame(coordinates(grid[[2]]), grid[[2]]$id,
                            extract(rasterstack, grid[[2]])) 

split.polys[2,]@polygons     
plot(split.polys[2,])
points(gridout@coords)

#Errors
ov <- sp::over(gridout, split.polys[2,])

plot(colocounties.UTM)
points(colorado.grid)
plot(polys[1,], border="blue", add=TRUE, lwd=10)
```
...
(distXsp[[1]][[4]]) #+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0   Need to transform

pca_herb <- coloradosps.g1g2
pca_herb <- pca_herb[!is.na(pca_herb$decimalLatitude),]

#change/transform to match projection for the raster stack
coordinates(pca_herb) <- c('decimalLongitude','decimalLatitude')
proj4string(pca_herb) <- CRS(rscrs)

#Clearly still have some of the really wrong, data entry problems
plot(pca_herb)

#Extract data from raster stack
#Get monthly 30 year average ppt, tmin, and tmax
data_herb <- data.frame(coordinates(pca_herb), pca_herb$id, 
                        pca_herb$genus, pca_herb$specificEpithet, 
                        extract(rasterstack, pca_herb))



#need to transform EOR grid points from CRS("+proj=utm +zone=13 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0") to rscrs ('raster stack CRS')
#Get the raster stack data from the EOR grid cells

pca_eor <- lapply(1:nrow(g1g2names), function(i){
  #put grid points across colorado, sample from it for each polygons
     polys <- l1G1G2[l1G1G2$GNAME %in% c(g1g2names$AcceptedName[i],
                                         g1g2names$Taxon[i]),] 
     grid <- makegrid(polys, cellsize = 100) # cellsize in map units!
     grid <- SpatialPointsDataFrame(grid, data=data.frame(id=1:nrow(grid),
                                                          acceptedName = g1g2names$AcceptedName[i],
                                                          taxon = g1g2names$Taxon[i]),
...
data_eor <- list()
for(i in 1:2){
  data_eor[[i]] <- data.frame(coordinates(pca_eor[[i]]), pca_eor[[i]]@data,
                              extract(rasterstack, pca_eor[[i]]))
}

data_eor <- lapply(EORsxsp, function(x){
  data.frame(coordinates(x), x$id, extract(rasterstack, x))
}) 
```
