...
l1G1G2_2 <- l1G1G2
proj4string(l1G1G2_2) <- CRS("+proj=utm +zone=13 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0")
...
plot(l1G1G2_2, border="gray60", add=TRUE, lwd=4) #
plot(projected, add=TRUE, pch=20, cex=0.5)
...
#Each polygon might be made of multiple disconnected polygons. Need to separate and label
table(g1g2names$Taxon)
table(l1eor$GRANK)
# Added G1G2 on 3/12/2019
l1G1G2 <- disaggregate(l1eor[l1eor$GRANK %in% c("G1","G2", "G1G2"),])
l1G1G2$PolyID <- do.call(rbind, lapply(split(l1G1G2,l1G1G2$OBJECTID),
                                       function(x){
              x$POlyID <- paste(x$EO_ID, LETTERS[seq(from=1, to= nrow(x))],sep="")
              x
...
plot(l1G1G2)
G1G2all <- as.data.frame(table(as.character(l1G1G2$GRANK),as.character(l1G1G2$GNAME)))
length(unique(G1G2all$Var2)) # 68 species with the G1G2s
g1g2names68 <- unique(G1G2all$Var2)
g1g2names68 <- TPL(g1g2names68)
g1g2names68$AcceptedName <- paste(g1g2names68$New.Genus,g1g2names68$New.Species,sep=" ")

```

#Poop, do I not have more, and we deleted all the polys?
```{r}

for(i in 1:68){
   polys <- l1G1G2[l1G1G2$GNAME %in% c(g1g2names68$AcceptedName[i],
                                        g1g2names68$Taxon[i]),] 
    proj4string(polys) <- CRS("+proj=utm +zone=13 ellps=NAD83 +ellps=WGS84") # CRS("+init=epsg:26913")
    
    print(paste(g1g2names68$AcceptedName[i],  g1g2names68$Taxon[i]))
    print(table(as.character(polys$LASTOBS)))
    print(as.numeric(substring(polys$LASTOBS,1,4)))
}

distXsp_noEORgrid68 <- lapply(1:nrow(g1g2names68), function(i){
  gc()
  g1g2now <- coloradosps.g1g2[coloradosps.g1g2$scientificName %in%
                                c(g1g2names68$Taxon[i],g1g2names68$AcceptedName[i])&
                                !is.na(coloradosps.g1g2$decimalLatitude),]
  ...
  out <- data.frame(Species = g1g2names68$AcceptedName[i], g1g2now,Dist = distnow, Area = totalarea, 
