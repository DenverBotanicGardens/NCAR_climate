...
Want to use elevation, slope and aspect. Could look at ruggedness too but don't have that yet. 
```{r}
...
i<-1
 polys <- l1G1G2[l1G1G2$GNAME %in% c(g1g2names$AcceptedName,
                                         g1g2names$Taxon),] 
 
length(polys)

sort(unique(polys@data$GNAME))[herbposition]
...
f1 <- lm(dist ~ as.factor(New.Genus)*year + area, data= distXsp1)
f2 <- lm(dist ~ year + area, data= distXsp1)
f3 <- lm(dist ~ year, data= distXsp1)
f4 <- lm(dist ~ area, data= distXsp1) #x is the area of the species known range
f5 <- lm(dist ~ 1, data= distXsp1)
f6 <- lm(dist ~ year*area, data= distXsp1) #year given the range
f7 <- lm(dist ~ as.factor(New.Genus)*(year), data= distXsp1)
...
```{r}
distXsp1$areabin <- 100
distXsp1$areabin[distXsp1$area < max(distXsp1$area)] <- 95
distXsp1$areabin[distXsp1$area < quantile(distXsp1$area, .75)] <- 75
distXsp1$areabin[distXsp1$area < quantile(distXsp1$area, .5)] <- 50
distXsp1$areabin[distXsp1$area < quantile(distXsp1$area, .25)] <- 25
distXsp1$areabin[distXsp1$area == min(distXsp1$area)] <- 1
table(distXsp1$areabin)

ggplot(distXsp1[distXsp1$dist<sort(distXsp1$dist,partial=(nrow(distXsp1))-2)[(nrow(distXsp1))-2]&
                distXsp1$area<=max(distXsp1$area),], aes(year, dist/1000))+
  geom_jitter(position=position_jitter(1), aes(colour=area/(1000^2)), size=4, alpha=0.95, pch=16)+  # geom_point(aes(colour=year))+
  stat_smooth(method="lm")+
  theme_bw()+
...
ggplot(distXsp1[distXsp1$dist<sort(distXsp1$dist,partial=(nrow(distXsp1))-2)[(nrow(distXsp1))-2]&
                distXsp1$area<=max(distXsp1$area),], aes(area/1000000, dist/1000))+
  geom_jitter(position=position_jitter(1.5), aes(colour=year), size=2, alpha=0.95, pch=21)+  # geom_point(aes(colour=year))+
  stat_smooth(method="lm")+
  theme_bw()+
  xlab("Species Range")+
  ylab("Distance (km) to nearest EOR")+
