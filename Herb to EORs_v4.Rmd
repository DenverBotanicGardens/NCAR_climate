...
stackmini <- stack(winPrecipclip,winTmaxclip)


#Loop through all the species
Maxent46species <- lapply(1:length(pca_data2), function(i){
  group_pa <- do.call(c,sapply(1:4, function(x) kfold(pca_data2[[i]][pca_data2[[i]]$pa==x,],10))) 
  
  out <- replicate){     #Are 10 repeats for 10 fold enough?
      #randomly select on of the test roups to sample as the training data, 10 fold
      test <- sample(1, 1:10)
      
      train_pEOR <- pca_data2[[i]][pca_data2[[i]]$pa == 1 & group_pa!=test,c("x1","x2")]
      train_pHerb <- pca_data2[[i]][pca_data2[[i]]$pa == 2 & group_pa!=test,c("x1","x2")]
      train_bEOR <- pca_data2[[i]][pca_data2[[i]]$pa == 3 & group_pa!=test,c("x1","x2")]
      train_bHerb <- pca_data2[[i]][pca_data2[[i]]$pa == 4 & group_pa!=test,c("x1","x2")]
      
      test_pEOR <- pca_data2[[i]][pca_data2[[i]]$pa == 1 & group_pa==test,c("x1","x2")]
      test_pHerb <- pca_data2[[i]][pca_data2[[i]]$pa == 2 & group_pa==test,c("x1","x2")]
      test_bEOR <- pca_data2[[i]][pca_data2[[i]]$pa == 3 & group_pa==test,c("x1","x2")]
      test_bHerb <- pca_data2[[i]][pca_data2[[i]]$pa == 4 & group_pa==test,c("x1","x2")]
