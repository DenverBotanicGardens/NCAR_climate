...
ggplot(distXsp1[distXsp1$dist<sort(distXsp1$dist,partial=(nrow(distXsp1))-2)[(nrow(distXsp1))-2]&
                distXsp1$area<=max(distXsp1$area),], aes(area, dist/1000))+
  geom_point()+
  theme_bw()+
...
ggplot(distXsp1[distXsp1$dist<sort(distXsp1$dist,partial=(nrow(distXsp1))-2)[(nrow(distXsp1))-2]&
                distXsp1$area<=max(distXsp1$area),], aes(area/1000000, dist/1000))+
  geom_jitter(position=position_jitter(1), aes(colour=year), size=4, alpha=0.95, pch=16)+  # geom_point(aes(colour=year))+
  stat_smooth(method="lm")+
  theme_bw()+
  xlab("Species Range")+
  ylab("Distance (km) to nearest EOR")+
