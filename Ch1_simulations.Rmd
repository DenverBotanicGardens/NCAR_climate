...
tolerancefunction <- function(rasterstack, specificitySD, backgroundSDmult, numinStack, N){
  ...
  centroid <- rnorm(N, mid.1, specificitySD*(max.1/100))
  background <- rnorm(N, mid.1, (specificitySD+backgroundSDmult)*(max.1/100)) # need to make sure specificitySD and backgroundSDmult don't add up to more than 100
  setClass("PresenceBackground", representation(Pr = "numeric", Bg = "numeric"))
  out <- new("PresenceBackground", Pr = centroid, Bg = background)
  out
}
...
simulateFundamentalNiche <- function(rasterstack, numinStack, N, specificitySD, backgroundSDmult){
  ...
  setClass("centroids", representation(SD = "numeric", SampleSize_N = "numeric", 
                                       Tolerance = "matrix", Background = "matrix"))
  out <- new("centroids", SD = specificitySD, SampleSize_N = N, 
             Tolerance = tolerance_variable)
  out
}
```
...
```{r}
simulatedspecies <- lapply(seq(20,100,by=20), function(x) simulateFundamentalNiche(rasterstack, 5, N = x, runif(5, 1, 100), backgroundSDmult = 5))
