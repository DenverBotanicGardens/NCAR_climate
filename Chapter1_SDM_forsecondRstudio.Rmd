...
habitatSpecificity <- function(whichones, pathstart, replicates){
  ...
  habspEOR <- do.call(rbind,lapply(whichones, function(x){
    ...
    EORpnts <- do.call(rbind, lapply(1:length(polys), function(f){
         ...
         grid <- makegrid(polys, cellsize = 500) # cellsize in map units!
         names(grid) <- c("x","y")
  ...
  habspecificity <- do.call(rbind,
                            lapply(whichones, function(x){
  ellipseoutwhichones <- do.call(rbind,
           lapply(unique(allsphabsp$HerbType[allsphabsp$SpeciesNum==x]),
                                          function(HT){
